...
public class ExcelTitleManager implements ISyncCatch {
    ...
    public static List<ExcelTitleBean> getExcelTitleList(Map<String, String> m) {
        ...
        return setExamName(ExcelTitleDAO.getExcelTitleList(m));
    }

    public static List<ExcelTitleBean> getAllExcelTitleList() {
        return setExamName(ExcelTitleDAO.getAllExcelTitleList());
    }

    static List<ExcelTitleBean> getExcelTitleListByExamID(String examId) {
        ...
        if (list.size() > 0) {
            for (ExcelTitleBean etb : list) {
                if (etb.getExamId() == Integer.parseInt(examId)) {
                    result.add(etb);
                }
            }
        }
        Collections.sort(result, new ExcelTitleComparatorForDESC());
        return setExamName(result);
    }


    public static ExcelTitleBean getExcelTitleBean(String id) {
        return setExamName(etMap.get(Integer.parseInt(id)));
    }
    ...
    public static ExcelTitleBean setExamName(ExcelTitleBean sb)
    {
    ...
    public static List<ExcelTitleBean> setExamName(List<ExcelTitleBean> scoreList)
    {
        if(scoreList != null && scoreList.size() > 0)
        {
        	List<ExamBean> examList = ExamManager.getAllExamList();
            for(ExcelTitleBean sb : scoreList)
            {
