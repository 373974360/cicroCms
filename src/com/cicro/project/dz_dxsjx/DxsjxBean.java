package com.cicro.project.dz_dxsjx;

/**
 * @Description:
 * @Author: like
 * @Date: 2018-06-14 11:53
 * @Version: 1.0
 * @Created in idea by autoCode
 */
public class DxsjxBean {
    private int id = 0;
    private String xxmc;    //学校名称
    private String sxzy;    //所学专业
    private String sznj;    //所在年级
    private String xm;    //姓名
    private String xb;    //性别
    private String mz;    //民族
    private String jg;    //籍贯
    private String zzmm;    //政治面貌
    private String jkzk;    //健康状况
    private String nl;    //年龄
    private String qq;    //QQ
    private String brtc;    //本人特长
    private String yzbm;    //邮政编码
    private String sfzhm;    //身份证号码
    private String lxdh;    //联系电话
    private String xxtxdz;    //学校通信地址
    private String jtlxrjdh;    //家庭联系人及电话
    private String jtxxzz;    //家庭详细住址
    private String hjqk;    //获奖情况
    private String grzjsq;    //个人自荐申请
    private String zp;    //照片
    private String tjb;    //电子版推荐表
    private String add_time;
    private String audit_time;
    private int status;
    private String user_id;

    public int getId() {
        return id;
    }

    public void setId(int id) {
        this.id = id;
    }

    public String getXxmc() {
        return xxmc;
    }

    public void setXxmc(String xxmc) {
        this.xxmc = xxmc;
    }

    public String getSxzy() {
        return sxzy;
    }

    public void setSxzy(String sxzy) {
        this.sxzy = sxzy;
    }

    public String getSznj() {
        return sznj;
    }

    public void setSznj(String sznj) {
        this.sznj = sznj;
    }

    public String getXm() {
        return xm;
    }

    public void setXm(String xm) {
        this.xm = xm;
    }

    public String getXb() {
        return xb;
    }

    public void setXb(String xb) {
        this.xb = xb;
    }

    public String getMz() {
        return mz;
    }

    public void setMz(String mz) {
        this.mz = mz;
    }

    public String getJg() {
        return jg;
    }

    public void setJg(String jg) {
        this.jg = jg;
    }

    public String getZzmm() {
        return zzmm;
    }

    public void setZzmm(String zzmm) {
        this.zzmm = zzmm;
    }

    public String getJkzk() {
        return jkzk;
    }

    public void setJkzk(String jkzk) {
        this.jkzk = jkzk;
    }

    public String getNl() {
        return nl;
    }

    public void setNl(String nl) {
        this.nl = nl;
    }

    public String getQq() {
        return qq;
    }

    public void setQq(String qq) {
        this.qq = qq;
    }

    public String getBrtc() {
        return brtc;
    }

    public void setBrtc(String brtc) {
        this.brtc = brtc;
    }

    public String getYzbm() {
        return yzbm;
    }

    public void setYzbm(String yzbm) {
        this.yzbm = yzbm;
    }

    public String getSfzhm() {
        return sfzhm;
    }

    public void setSfzhm(String sfzhm) {
        this.sfzhm = sfzhm;
    }

    public String getLxdh() {
        return lxdh;
    }

    public void setLxdh(String lxdh) {
        this.lxdh = lxdh;
    }

    public String getXxtxdz() {
        return xxtxdz;
    }

    public void setXxtxdz(String xxtxdz) {
        this.xxtxdz = xxtxdz;
    }

    public String getJtlxrjdh() {
        return jtlxrjdh;
    }

    public void setJtlxrjdh(String jtlxrjdh) {
        this.jtlxrjdh = jtlxrjdh;
    }

    public String getJtxxzz() {
        return jtxxzz;
    }

    public void setJtxxzz(String jtxxzz) {
        this.jtxxzz = jtxxzz;
    }

    public String getHjqk() {
        return hjqk;
    }

    public void setHjqk(String hjqk) {
        this.hjqk = hjqk;
    }

    public String getGrzjsq() {
        return grzjsq;
    }

    public void setGrzjsq(String grzjsq) {
        this.grzjsq = grzjsq;
    }

    public String getZp() {
        return zp;
    }

    public void setZp(String zp) {
        this.zp = zp;
    }

    public String getTjb() {
        return tjb;
    }

    public void setTjb(String tjb) {
        this.tjb = tjb;
    }

    public String getAdd_time() {
        return add_time;
    }

    public void setAdd_time(String add_time) {
        this.add_time = add_time;
    }

    public String getAudit_time() {
        return audit_time;
    }

    public void setAudit_time(String audit_time) {
        this.audit_time = audit_time;
    }

    public int getStatus() {
        return status;
    }

    public void setStatus(int status) {
        this.status = status;
    }

    public String getUser_id() {
        return user_id;
    }

    public void setUser_id(String user_id) {
        this.user_id = user_id;
    }
}

