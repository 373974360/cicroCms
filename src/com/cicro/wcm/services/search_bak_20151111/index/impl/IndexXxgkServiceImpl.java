 ...
 * @version 1.0
 */
public class IndexXxgkServiceImpl implements IndexService {

	static int s_count = 50;//每次从数据库中读出50条
	static IIndexInfoDao indexInfoDao = new IndexXxgkDaoImpl();
	...
	public boolean appendALlDocument(Map mapSite) {
		...
		try{
			 ...
			 for(int k=0;k<page;k++){
	    		  ...
	    		  List<Map> infoList = s.selectList(DBManager.getSqlNameByDataType("search.getXxgkListBySiteIdPages"),mapSite);
	...
	public boolean appendSingleDocument(Map infos) {
		...
		try{
			...
			if(infos.get("id")!=null){
				...
				mapC.put("id",id);
				Map map = (Map)s.selectOne(DBManager.getSqlNameByDataType("search.getXxgkById"), mapC);
