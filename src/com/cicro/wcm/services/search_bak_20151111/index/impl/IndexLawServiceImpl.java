package com.cicro.wcm.services.search_bak_20151111.index.impl;
 ...
 * @version 1.0
 */
public class IndexLawServiceImpl implements IndexService {

	static int s_count = 50;//每次从数据库中读出50条
	static IIndexInfoDao indexInfoDao = new IndexLawDaoImpl();
	...
	public boolean appendALlDocument(Map mapSite) {
		...
		try{
			 ...
			 //得到indexWriter
			 indexWriter = IndexLuceneManager.initIndexWriter();
			 ...
			 for(int k=0;k<page;k++){
	    		  ...
	    		  List<Map> infoList = s.selectList(DBManager.getSqlNameByDataType("search.getLawListBySiteIdPages"),mapSite);
	...
	public boolean appendSingleDocument(Map infos) {
		...
		try{
			...
			if(infos.get("id")!=null){
				...
				mapC.put("id",id);
				Map map = (Map)s.selectOne(DBManager.getSqlNameByDataType("search.getLawById"), mapC);
...
//				    	  }
			  ...
			  indexBeanXxgk.setId(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("info_id")));
			  indexBeanXxgk.setTitle(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("title")));
			  indexBeanXxgk.setApp_id(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("app_id")));
			  StringBuffer sb = new StringBuffer();
			  sb.append(FormatUtil.formatNullString((String)infoMap.get("gk_xzxw"))); 
			  sb.append(FormatUtil.formatNullString((String)infoMap.get("gk_xzcl")));
			  sb.append(FormatUtil.formatNullString((String)infoMap.get("gk_flyj")));
			  sb.append(FormatUtil.formatNullString((String)infoMap.get("gk_nrfj")));
			  indexBeanXxgk.setContent(HtmlRegexpUtil.filterHtml(sb.toString())); 
			  indexBeanXxgk.setUrl(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("content_url")));
			  String site_id = com.cicro.wcm.services.control.site.SiteAppRele.getSiteIDByAppID("zwgk");
			  indexBeanXxgk.setSite_id(site_id);   
			  
			  indexBeanXxgk.setNode_id(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("site_id")));
			  
			  indexBeanXxgk.setModel_id(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("model_id")));
			  
			  indexBeanXxgk.setWnumber(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("doc_no")));
			  indexBeanXxgk.setCode(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("gk_index")));
			  String time = ((String)infoMap.get("released_dtime"));
			  time = time == null ? "" : time.replaceAll("-","").replaceAll(" ", "").replaceAll(":","");
			  indexBeanXxgk.setPublish_time(com.cicro.util.FormatUtil.formatNullString(time)); 
			  time = ((String)infoMap.get("generate_dtime"));
			  time = time == null ? "" : time.replaceAll("-","").replaceAll(" ", "").replaceAll(":","");
			  indexBeanXxgk.setC_time(com.cicro.util.FormatUtil.formatNullString(time));
			  time = ((String)infoMap.get("effect_dtime"));
			  time = time == null ? "" : time.replaceAll("-","").replaceAll(" ", "").replaceAll(":","");
			  indexBeanXxgk.setTake_time(com.cicro.util.FormatUtil.formatNullString(time)); 
			  time = ((String)infoMap.get("aboli_dtime")); 
			  time = time == null ? "" : time.replaceAll("-","").replaceAll(" ", "").replaceAll(":","");
			  indexBeanXxgk.setOver_time(com.cicro.util.FormatUtil.formatNullString(time)); 
			  indexBeanXxgk.setObject_words(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("topic_key"))+" "+com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("place_key")));
			  indexBeanXxgk.setForm_type(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("theme_name")));
			  indexBeanXxgk.setContent_type(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("topic_name")));
			  indexBeanXxgk.setPublish_org(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("gk_input_dept")));
			  indexBeanXxgk.setCategoryId(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("cat_id")));
			  indexBeanXxgk.setIs_host(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("is_host")));
