 ...
 * @version 1.0
 */
public class IndexOrgServiceImpl implements IndexService {

	static int s_count = 50;//每次从数据库中读出50条
	static IIndexInfoDao indexInfoDao = new IndexOrgDaoImpl();
	...
	public boolean appendALlDocument(Map mapSite) {
		...
		try{
			 ...
			 for(int k=0;k<page;k++){
	    		  ...
	    		  List<Map> infoList = s.selectList(DBManager.getSqlNameByDataType("search.getOrgListBySiteIdPages"),mapSite);
	...
	public boolean appendSingleDocument(Map infos) {
		...
		try{
			...
			if(infos.get("id")!=null){
				...
				mapC.put("id",id);
				Map map = (Map)s.selectOne(DBManager.getSqlNameByDataType("search.getOrgById"), mapC);
...
//				    	  }
			  ...
			  indexBeanXxgk.setApp_id(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("add_id")));
			  StringBuffer sb = new StringBuffer();
			  sb.append(FormatUtil.formatNullString((String)infoMap.get("gk_gzzz"))); 
			  sb.append(FormatUtil.formatNullString((String)infoMap.get("gk_fzr")));
			  sb.append(FormatUtil.formatNullString((String)infoMap.get("gk_bgdz")));
