 ...
 * @version 1.0
 */
public class IndexCustomGkServiceImpl implements IndexService {

	static int s_count = 50;//每次从数据库中读出50条
	static IIndexInfoDao indexInfoDao = new IndexCustomGkDaoImpl();
			...
			//得到所有的自定义表  列表 --- start
			 ...
			 for(ModelBean modelBean : modelBeanList){
				 ...
				 for(int k=0;k<page;k++){
		    		  ...
		    		  List<Map> infoList = s.selectList(DBManager.getSqlNameByDataType("search.getCostomGkListBySiteIdPages"),mapSite);
	...
	public boolean appendSingleDocument(Map infos) {
		...
		try{
			...
			if(infos.get("id")!=null){
				...
				Map map = (Map)s.selectOne(DBManager.getSqlNameByDataType("search.getCustomGkById"), maptemp);
...
//				    }
			  ...
			  indexBeanXxgk.setContent(HtmlRegexpUtil.filterHtml(FormatUtil.formatNullString(sbCustom.toString())));
			  //indexBeanXxgk.setContent(HtmlRegexpUtil.filterHtml(com.cicro.util.FormatUtil.formatNullString((String)infoMap.get("gk_content"))));
