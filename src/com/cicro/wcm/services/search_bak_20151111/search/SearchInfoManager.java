package com.cicro.wcm.services.search_bak_20151111.search;

import com.cicro.analyzer.lucene.IKQueryParser;
...
//			QueryParser parser = new MultiFieldQueryParser(Version.LUCENE_30, fields, new IKAnalyzer(true));
//			query = parser.parse(q);
    ...
    public static SearchResult search(Map map) {
        ...
        try {
            ...
            //过滤条件
            QueryParser parser = new MultiFieldQueryParser(Version.LUCENE_30, new String[]{""}, new com.cicro.analyzer.lucene.IKAnalyzer());
...
//			System.out.println("sn+pz===" + (Integer.valueOf(sn)+Integer.valueOf(pz)));
            ...
            for (int j = sn; j < hits.scoreDocs.length; j++) {
                ...
                TokenStream tokenStream = new com.cicro.analyzer.lucene.IKAnalyzer().tokenStream("token", new StringReader(content));
...
//				TokenStream tokenStream = new StandardAnalyzer(Version.LUCENE_30).tokenStream("token",new StringReader(content));
                ...
                String url = doc.get("url");
                String title = doc.get("title");
                tokenStream = new com.cicro.analyzer.lucene.IKAnalyzer().tokenStream("token", new StringReader(title));
