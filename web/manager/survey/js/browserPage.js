...
function scaleReady()
{
	$("UL[stype=img] LI").hover(
	  ...
	  function () {
		...
		if(ULObj.find("LI[checked=true]").length >0)
		{
			changeScaleLIClass(ULObj.find("LI[checked=true]"));
		}else{
...
{			
	...
	$("div[sort_num]").each(function(i){		
			...
			{
				var item_id = $(this).find(":radio[checked=true]").attr("id");
				...
				if(value != null)
				{
					...
					var textObj =  $(this).find(":radio[checked=true]").parent().find(":text");
					if(textObj.is("input"))
					{
			...
			if(type == "scale")
			{
				...
				{
					item_id = $(this).find("LI[checked=true]").attr("id");
					value = $(this).find("LI[checked=true]").attr("value");
				}
				else
				{
					item_id = $(this).find(":radio[checked=true]").attr("id");
					value = $(this).find(":radio[checked=true]").val();
				}
	...
	{		
		...
		var result = AnswerRPC.insertSurveyAnswer(SurveyAnswer,auth_code);
		if(result == "true")
		{
			 ...
			 if(childList != null && childList.size() > 0)
			 {
				for(var j=0;j<childList.size();j++)
				{
					...
					str += '<tr>';
					 ...
					 str += '<td id="pic_'+ids+'" style="text-align:left" width="155px"><div class="pro_back"><div class="pro_fore" style="width:'+getStatisBean(ids).proportion+'"><img height="13" width="149px" alt="" src="/manager/images/pro_fore.png"></div></div></td>'
					...
					if(childList != null && childList.size() > 0)
					{
						for(var j=0;j<childList.size();j++)
						{
							...
							str += '<tr item_id="'+SurveySuvItem.item_id+'">';
							 ...
							 str += '<td id="pic_'+ids+'" style="text-align:left" width="155px"><div class="pro_back"><div class="pro_fore" style="width:0%"><img height="13" width="149" alt="" src="/manager/images/pro_fore.png"></div></div></td>'
