	...
	var SurveyCategory = new Bean("com.cicro.wcm.bean.survey.SurveyCategory",true);
	var SurveyBean = new Bean("com.cicro.wcm.bean.survey.SurveyBean",true);
	var SurveySub = new Bean("com.cicro.wcm.bean.survey.SurveySub",true);
	var SurveySuvItem = new Bean("com.cicro.wcm.bean.survey.SurveySuvItem",true);
	var ChildItem = new Bean("com.cicro.wcm.bean.survey.SurveyChildItem",true);
...
function publishSurvey(publish_status)
{
	...
	if(SurveyRPC.publishSurvey(beanList.get(carent_cell_num).id,publish_status,top.LoginUserBean.user_id))
	{
...
function batchPublishSurvey(publish_status)
{
	...
	if(selectIDS == "")
	{
		top.msgWargin("请选择要"+message+"的记录");
		return;
	}

	var selectIDS = table.getSelecteCheckboxValue("id");
	
	if(SurveyRPC.publishSurvey(selectIDS,publish_status,top.LoginUserBean.user_id))
	{
		...
		top.msgWargin(WCMLang.Publish_success);
}
/**********************发布操作　结束*************************************/
...
{
	top.msgConfirm("确认删除选中记录？","deleteSurvey("+id+")");
}

function deleteSurvey(id)
{
	if(SurveyRPC.deleteSurvey(id,top.LoginUserBean.user_id))
	{
		top.msgAlert("问卷调查主题"+WCMLang.Delete_success);		
		showTurnPage();
		showList();		
	}
	else
		top.msgWargin("问卷调查主题"+WCMLang.Delete_fail);
}

function batchDelSurvey()
{
	if (table.getSelecteCount() < 1) {
			top.msgWargin("请选择要删除的记录！");
			return;
	}

	top.msgConfirm("确认删除选中记录？","batchDelSurveyHandl()");
}
...
function batchDelSurveyHandl()
{
	...
	if(SurveyRPC.deleteSurvey(selectIDS,top.LoginUserBean.user_id))
	{
		top.msgAlert("问卷调查主题"+WCMLang.Delete_success);		
		showTurnPage();
		showList();	
...
{
	var selectIDS = table.getSelecteCheckboxValue("s_id");
	window.open('/manager/survey/answerPage.jsp?sid='+selectIDS);	
}
...
function openSubject()
{
	...
	var selectIDS = table.getSelecteCheckboxValue("s_id");
	top.addTab(true,'/manager/survey/statisticsSubject.jsp?sid='+selectIDS,'数据统计');
}
...
function openAnswer()
{
	...
	var selectIDS = table.getSelecteCheckboxValue("s_id");
	top.addTab(true,'/manager/survey/answerList.jsp?sid='+selectIDS,'查看答卷');
}
/**********************属性设置操作　开始*************************************/
function showSurveyAttr(flag)
{
	...
	{
		var selectIDS = table.getSelecteCheckboxValue("s_id");
		top.addTab(true,'/manager/survey/setSurveyAttr.jsp?s_id='+selectIDS+"&top_index="+top.curTabIndex,'问卷调查属性设置');	
	}	
}
...
function setSurveyAttrHandl()
{
	...
	{
		top.msgAlert("保存成功");
		top.getCurrentFrameObj(top_index).showList();
		top.tab_colseOnclick(top.curTabIndex)
	}else
	{
		top.msgAlert("保存失败,请重新提交");
		return;
	}	
}
...
function fnSearch()
{	
	//top.addTab(true,'/manager/survey/search_survey.jsp','问卷调查查询');	
	top.OpenModalWindow("问卷调查查询","/manager/survey/search_survey.jsp",455,245);	
}
