		...
		}else
		{
			...
			$(li).bind('click', function(event) {
				...
				if($(event.target).data("attributes").url != null && $(event.target).data("attributes").url != "")
					...
					setMainIframeUrl("/manager/space.html","blank_"+tabIndex);
...
{	
	...
	$('#sysDialog_Frame').attr("src","/manager/spaceIframe.html");
}
...
function getInfoCategoryTreeByUserID()
{
	...
	try{
	setTimeout(function(){
		jsonrpc.CategoryRPC.getInfoCategoryTreeByUserID(reloadLeftMenuTree,current_site_id,LoginUserBean.user_id)
	},10);
	...
	*/
	...
	$('#leftMenuTree').tree({   
		 checkbox: false,   
		 url: '/admin/servlet/Category?site_id=' + current_site_id + '&user_id=' + LoginUserBean.user_id + '&pid=0',   
		 onBeforeExpand:function(node,param){
			 $('#leftMenuTree').tree('options').url = '/admin/servlet/Category?site_id=' + current_site_id + '&user_id=' + LoginUserBean.user_id + '&pid=' + node.id;	// change the url                       
