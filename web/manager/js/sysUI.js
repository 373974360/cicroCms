...
function init_input()
{
	...
	$("label").click(function(){		
		if($(this).prev("input").attr("checked")  == true || $(this).prev("input").attr("checked")  == "checked")
...
{
	window.showModalDialog("/manager/material/operate/select_material.jsp?i_type="+i_type+"&site_id="+site_id+"&handl_name="+handl_name,window,'dialogWidth:700px;dialogHeight:550px;scroll:no;status:no;center:yes');
}
...
{
	top.OpenModalWindow(title,"/manager/org/dept/select_single_dept.jsp?handl_name="+handl_name+"&stype="+show_type,450,510);
}

//打开站点选择窗口
function openSelectSingleSite(title,handl_name)
{
	top.OpenModalWindow(title,"/manager/control/site/select_single_site.jsp?handl_name="+handl_name,450,510);
}

//打开人员选择窗口
function openSelectUserPage(title,handl_name)
{
	top.OpenModalWindow(title,"/manager/org/dept/select_user.jsp?handl_name="+handl_name,720,515);
}

//打开站点下的用户和用户组选择窗口
function openSelectSiteUserPage(title,handl_name,site_id)
{
	top.OpenModalWindow(title,"/manager/org/siteUser/select_site_user.jsp?app_id="+app_id+"&site_id="+site_id+"&handl_name="+handl_name,520,515);
}

//打开站点下的用户
function openSelectOnlySiteUserPage(title,handl_name,site_id)
{
	top.OpenModalWindow(title,"/manager/org/siteUser/select_site_only_user.jsp?site_id="+site_id+"&handl_name="+handl_name,520,505);
}
...
{
	top.OpenModalWindow(title,"/manager/appeal/cpUser/select_user.jsp?handl_name="+handl_name,720,515);
}

//打开诉求部门选择窗口
function openSelectCpDeptPage(title,handl_name)
{
	top.OpenModalWindow(title,"/manager/appeal/cpDept/select_cpDept.jsp?handl_name="+handl_name,610,510);
}

//打开权限选择窗口
function openSelectOperatePage(title,handl_name,update_status)
{	
	top.OpenModalWindow(title,"/manager/org/operate/select_operate.jsp?handl_name="+handl_name+"&update_status="+update_status,450,510);
}
...
{
	top.OpenModalWindow(title,"/manager/org/role/select_role.jsp?app_id="+app_id+"&site_id="+site_id+"&handl_name="+handl_name,450,510);
}
...
function openSetOperate(title,handl_name,app_id,site_id,priv_type,u_id)
{
	...
	if(u_id == null)
	{
		...
		user_id = u_id; 
	top.OpenModalWindow(title,"/manager/org/siteUser/set_operate.jsp?app_id="+app_id+"&priv_type="+priv_type+"&user_id="+user_id+"&handl_name="+handl_name,550,530);
}

//打开目录选择窗口
function openSelectSingleCategory(title,handl_name,site_id)
{
	top.OpenModalWindow(title,"/manager/cms/category/select_single_category.jsp?site_id="+site_id+"&handl_name="+handl_name,450,510);
}

//打开基础分类法分类方式类目选择窗口
function openSelectSingleCateClass(title,handl_name)
{
	top.OpenModalWindow(title,"/manager/cms/cateClass/select_single_cateClass.jsp?class_type=0&handl_name="+handl_name,450,510);
}

//打开共享目录分类方式类目选择窗口
function openSelectSingleShareCateClass(title,handl_name,app_id)
{
	top.OpenModalWindow(title,"/manager/cms/cateClass/select_single_cateClass.jsp?class_type=1&app_id="+app_id+"&handl_name="+handl_name,450,510);
}

//打开共享目录分类方式类目选择窗口
function openSelectShareCateClass(title,handl_name,app_id)
{
	top.OpenModalWindow(title,"/manager/cms/cateClass/select_cateClass.jsp?app_id="+app_id+"&handl_name="+handl_name,450,500);
}

//打开公开节点分类选择窗口
function openSelectGKNodeCategory(title,handl_name,show_tree_type)
{
	top.OpenModalWindow(title,"/manager/zwgk/node/select_node_category.jsp?stt="+show_tree_type+"&handl_name="+handl_name,450,500);
}
...
function openSelectTemplate(template_id,template_name,site_id)
{
	...
	top.OpenModalWindow("选择模板","/manager/system/template/select_template.jsp?site_id="+site_id+"&handl_name=setSelectTemplate",520,500);
}
