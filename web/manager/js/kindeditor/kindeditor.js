...
function _formatHtml(html, htmlTags, urlType, wellFormatted, indentChar) {
	...
	html = html.replace(re, function($0, $1, $2, $3, $4, $5, $6) {
		...
		if(startSlash == "/")
		{
			return '<' + startSlash + tagName + attr + endSlash + '>' + endNewline;
		}else
			return '<' + startSlash + tagName + attr + endSlash + '>';
...
/*************添加的内容 开始 kisslan**************/
//修改记录
//由于新版对代码格式添加了换行，导致翻页不能替换，现去掉换行的代码
//861行加了一个判断的代码


var ke_map = new Map();//用于存储textareaid的，新版本的编辑器不支持用id直接取，只能用instances数组取，这个用来存储ＩＤ和对应的下标
var ke_count = 0;
//迷你编辑器
function init_editer_min(textareaID)
{
	ke_map.put(textareaID,ke_count);
	ke_count += 1;
	KindEditor.ready(function(K) {
				K.create('textarea[id="'+textareaID+'"]', {
					
					items : [
					'undo', 'redo', 'cut', 'copy', 'paste',
					'plainpaste', 'wordpaste', '|', 'insertorderedlist', 'insertunorderedlist', '|',
					 'fontsize', 'textcolor',  'bold',
					'italic', 'underline', '|','advtable', '|','selectall','formatblock']
				});
			});
	setHtmlEditButton(textareaID);	
}
//简易
function init_editer_simple(textareaID)
{
	ke_map.put(textareaID,ke_count);
	ke_count += 1;
	KindEditor.ready(function(K) {
		...
		'plainpaste', 'wordpaste', '|', 'insertorderedlist', 'insertunorderedlist', '|',
		 'fontsize', 'textcolor',  'bold',
		'italic', 'underline', '|','image', '|','advtable', '|','selectall','formatblock']
				});
			});
}
//图片
function init_editer_img(textareaID)
{
	KindEditor.ready(function(K) {
				K.create('textarea[id="'+textareaID+'"]', {
		 ...
		 'fontsize', 'textcolor',  'bold',
		'italic', 'underline','image',  '|','selectall']
				});
			});
}
...
function init_editer(textareaID)
{
	...
	KindEditor.ready(function(K) {
		...
		'formatblock', 'fontname', 'fontsize', '|', 'forecolor', 'hilitecolor', 'bold',
		'italic', 'underline', 'strikethrough', 'lineheight', 'removeformat', '|', 'image', 'multiimage',
		'flash', 'media', 'insertfile', 'table', 'hr', 'baidumap', 
		'anchor', 'link', 'unlink']
				});
			});
	setHtmlEditButton(textareaID);
...
//带翻页的全编辑器
function init_editer_turnpage(textareaID)
{
	ke_map.put(textareaID,ke_count);
	ke_count += 1;
	KindEditor.ready(function(K) {
...
function setHtmlEditButton(textareaID)
{
	...
	{
		var str = '<div style="padding-top:3px"><input id="tool_btn" type="button" value="WORD格式清理" onclick="formatStyle(\'clearWordStyle\',\''+textareaID+'\')" style="cursor:hand"/>&#160;<input id="tool_btn"  type="button" value="清除样式" onclick="formatStyle(\'clearStyle\',\''+textareaID+'\')"  style="cursor:hand"/>&#160;<input id="tool_btn" type="button" value="去除空行" onclick="formatStyle(\'clearBlankRow\',\''+textareaID+'\')"  style="cursor:hand"/>&#160;<input id="tool_btn" type="button" value="首行缩进" onclick="formatStyle(\'firstRowIndent\',\''+textareaID+'\')"  style="cursor:hand"/>&#160;<input id="tool_btn"  type="button" value="全角转半角" onclick="formatStyle(\'qjTObj\',\''+textareaID+'\')"  style="cursor:hand"/>&#160;<input id="tool_btn"  type="button" value="一键排版" onclick="formatStyle(\'yjpb\',\''+textareaID+'\')"  style="cursor:hand"/></div>';
		$("#"+textareaID).after(str);
		init_input();	
	}
}
//
...
//得到编辑器对象
function getKEditorObject(textareaID)
{
	return KindEditor.instances[ke_map.get(textareaID)];
}

function KEditorForCustom(){
	this.html = function(textareaID,content){
		if(content != null)
		{
			try{
				KindEditor.instances[ke_map.get(textareaID)].html(content);
			}catch(e){}
		}else
		{
			try{
				return KindEditor.instances[ke_map.get(textareaID)].html();
			}catch(e){}
		}
	};
	this.util = function(textareaID){
		return KindEditor.instances[ke_map.get(textareaID)];
	};
};
var KE = new KEditorForCustom();

function formatStyle(handlName,textareaID)
{
	var str = KE.html(textareaID);
	
	switch(handlName){
		case 'clearBlankRow':
			str = clearBlankRow(str);
			break;
		case 'firstRowIndent':
			str = firstRowIndent(str);
			break;
		case 'qjTObj':
			str = qjTObj(str);
			break;
		case 'clearStyle':
			str = clearStyleByTag(str);
			KE.html(textareaID,str);			
			KE.util(textareaID).focus();
			KE.util(textareaID).exec("selectall");			
			KE.util(textareaID).exec("removeformat");
			KE.util(textareaID).focus();			
			return;
		case 'clearWordStyle':
...
function clearStyleByTag(str)
{
	try{
		return str.replace(/(<[^>]*?)(style=\s*[\"|\'].*?\s*[\"|\'])([^>]*>)/ig,"$1$3").replace(/<(\/)?h[1-9]>/g,"<$1p>").replace(/(<[^>]*?)(center=\s*[\"|\'].*?\s*[\"|\'])([^>]*>)/ig,"$1$3").replace(/<(\/)?h[1-9]>/g,"<$1p>");
	}catch(e){
		return str;
	}	
}

function clearBlankRow(str){
	...
	for(var i=0;i<vPatterns.length;i++){
		...
		str=str.replace(vRegExp,vReplaces[i]);
	}
	return str;
}

function firstRowIndent(str){	
	var regReplace = /(<p(|\s+[^>]*)>)((\n|\r|.)*?<\/p>)/img;
	/*return str.replace(regReplace, function(){
		return arguments[1] + '　　' + 
	...
	return str.replace(regReplace, function(){
		/*return arguments[1] + '　　' + 
			arguments[3].replace(/^(<(strong|em|i)>)(&nbsp;|&nbsp|　||\s)+/ig, '$1')
				.replace(/^(&nbsp;|&nbsp|　||\s)+/g, '');*/
		return arguments[1].replace(">"," style='text-indent:2em;'>")+arguments[3].replace(/^(<(strong|em|i)>)(&nbsp;|&nbsp|　||\s)+/ig,'$1').replace(/^(&nbsp;|&nbsp|　||\s)+/g, '');
	});	
}
...
function qjTObj(str){
	...
	var vTemp=null;
	var lastIndex=0;
	while((vTemp=vMatchAll.exec(str))!=null){
		vReturn+=str.substring(lastIndex,vTemp.index);
		vReturn+=qjCharTOChar(vTemp[0]);
		lastIndex=vTemp.index+vTemp[0].length;
	}
	if(lastIndex<str.length){
		vReturn+=str.substring(lastIndex);
	}
	return vReturn;
}

function qjCharTOChar(s) {
    return String.fromCharCode(s.charCodeAt(0)-65248); 
}

function clearWordStyle( sHtml, bIgnoreFont, bRemoveStyles, bRemoveEmptyBlock)
{
