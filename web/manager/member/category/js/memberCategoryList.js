...
var MemberCategoryBean = new Bean("com.cicro.wcm.bean.member.MemberCategoryBean",true);
...
function showList(){
	...
	table.getCol("mcat_name").each(function(i){	
		if(i>0)
		{
			...
			$(this).html('<a href="javascript:top.OpenModalWindow(\'会员分类信息\',\'/manager/member/category/memberCategory_add.jsp?type=update&mcat_id='+id +'\',450,245)">'+beanList.get(i-1).mcat_name+'</a>');	
		}
	});
...
{
	top.OpenModalWindow("新建会员分类信息","/manager/member/category/memberCategory_add.jsp?type=add&mcat_id=",460,245);
}
...
function updateMemberCategory()
{
	...
	top.OpenModalWindow("修改会员分类信息","/manager/member/category/memberCategory_add.jsp?type=update&mcat_id="+id,460,245);
}
