...
var MemberBean = new Bean("com.cicro.wcm.bean.member.MemberBean",true);
var MemberRegisterBean = new Bean("com.cicro.wcm.bean.member.MemberRegisterBean",true);
...
function showList(){
	...
	table.getCol("me_realname").each(function(i){	
		if(i>0)
		{
			...
			$(this).html('<a href="/manager/member/manager/member_view.jsp?type=update&member_id='+id+'">'+beanList.get(i-1).me_realname+'</a>');
		}
	});
...
function addMember()
{
	window.location.href  = "/manager/member/manager/member_view.jsp?type=add&member_id=";
}
...
function updateStatus()
{
	...
	top.OpenModalWindow("会员维护","/manager/member/manager/member_status.jsp?member_id="+id,350,210);
}
...
{
	var id = table.getSelecteCheckboxValue("me_id");
	window.location.href  = "/manager/member/manager/member_view.jsp?type=update&member_id="+id;
}
