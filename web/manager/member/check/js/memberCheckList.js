...
function showList(){
	...
	table.getCol("me_realname").each(function(i){	
		if(i>0)
		{
			...
			$(this).html('<a href="javascript:top.addTab(true,\'/manager/member/manager/member_view.jsp?type=view&member_id='+id+'\',\'会员信息\')">'+beanList.get(i-1).me_realname+'</a>');
		}
	});
...
function singleCheck()
{
	...
	top.OpenModalWindow("会员审核","/manager/member/check/member_check.jsp?member_id="+ids+"&type=single",350,190);
...
{
	var ids = table.getSelecteCheckboxValue("me_id");
	top.OpenModalWindow("会员批量审核","/manager/member/check/member_check.jsp?member_id="+ids+"&type=batch",350,140);
}
