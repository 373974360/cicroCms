...
var MateFolderBean = new Bean("com.cicro.wcm.bean.material.MateFolderBean",true);
var MateInfoRPC = jsonrpc.MateInfoRPC;
var MateInfoBean = new Bean("com.cicro.wcm.bean.material.MateInfoBean",true);
...
function initMenuTree()
{
	$('#leftMenuTree').tree({		
		onClick:function(node)
		{
			...
			{
				var time="";
				$("#SucaiListArea").attr("style","display:block");
				$("#folderListDiv").attr("style","display:none");
				$("#tableoption").attr("style","display:block");
...
function optionIsDisabled()
{
	...
	$("#SucaiListArea").attr("style","display:none");
	$("#tableoption").attr("style","display:none");
	$("#folderListDiv").attr("style","display:block");
}
//置灰的放开
...
function showMateInfosList(f_id,site_id,ctype,imgDomain,page_size,time)
{
	...
	for(var i=0;i<beanList.size();i++)
	{
		...
		var t_thumb_url = imgDomain+beanList.get(i).att_path + beanList.get(i).thumb_url;
...
function showPreviewBySuffix(f_url)
{
	...
	var flash_ext = "swf,flv";
	var media_ext = "asf,avi,mpg,mpeg,mpe,mov,rmvb,wmv,wav,mid,midi,mp3,mpa,mp2,ra,ram,rm,wma";
	var zip_ext = "zip,rar,jar,tar";
	var txt_ext = "txt,doc,docx,xls,html,htm";

	if(img_ext.indexOf(extName) > -1)
	{
		return f_url;
	}
	if(flash_ext.indexOf(extName) > -1)
	{
		return "../../images/ext/flash.png";
	}
	if(media_ext.indexOf(extName) > -1)
	{
		return "../../images/ext/media.png";
	}
	if(zip_ext.indexOf(extName) > -1)
	{
		return "../../images/ext/zip.png";
	}
	if(txt_ext.indexOf(extName) > -1)
	{
		return "../../images/ext/txt.png";
	}
...
function MateInfoClick(type,site_id){
	...
	if(type == "1"){
		if(Temparr.length > 1){
			...
			top.OpenModalWindow("素材信息","/manager/material/operate/mateInfo_modify.jsp?att_id="+str+"&cnode_id="+cnode_id+"&cnode_type="+cnode_type+"&site_id="+site_id,500,210);
		}
	}else if(type == "2")
	{
...
function MateFolderClick(site_id,type){	
	...
	if(type == "1"){
		...
		{ 
			...
			top.OpenModalWindow("维护目录","/manager/material/operate/matefolder_add.jsp?f_id="+f_id+"&site_id="+site_id,470,195);
		}
	}else if(type == "2"){
		...
		{ 
			...
			top.OpenModalWindow("维护目录","/manager/material/operate/matefolder_modify.jsp?f_id="+tempid,470,130);
		}
	}else if(type == "3"){
		deleteCustomFollder();
	}
	...
	$('#SucaiListArea').scroll(function(){
		...
		if (o.scrollTop()+o.height() > o.get(0).scrollHeight - 90)
		{
			...
			if (start_num >= sucaiFolderCount) return;
				...
				showMateInfosList(tempid,site_id,"custom",imgDomain,pageSize);
...
{	
	...
	if(move_selected_id == "" || move_selected_id  == null)
	{
		...
		top.OpenModalWindow("选择目录","/manager/material/operate/cat_tree.jsp?site_id="+site_id,350,420);	
}
