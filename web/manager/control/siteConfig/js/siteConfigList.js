...
var SiteConfigBean = new Bean("com.cicro.wcm.bean.control.SiteConfigBean",true);
...
function openAddPage(){ 
	top.OpenModalWindow("维护配置","/manager/control/siteConfig/siteConfig_add.jsp",500,160);
}

//打开修改页面
function openUpdatePage(){
	var selectIDS = table.getSelecteCheckboxValue("config_id");
	top.OpenModalWindow("维护配置","/manager/control/siteConfig/siteConfig_add.jsp?config_id="+selectIDS,500,160);
}
...
function funAdd(){
	...
	{
		top.msgAlert("站点配置信息"+WCMLang.Add_success);
		top.CloseModalWindow();
		top.getCurrentFrameObj().reloadDataList();
	}
	else
	{
		top.msgWargin("站点配置信息"+WCMLang.Add_fail);
	}
}
...
function funDelete(){ 
	...
	if(SiteConfigRPC.deleteSiteConfig(selectIDS)){
		top.msgAlert("站点配置信息"+WCMLang.Delete_success);
		top.getCurrentFrameObj().reloadDataList(); 
	}else
	{
		top.msgWargin("站点配置信息"+WCMLang.Delete_fail);
	}
}
