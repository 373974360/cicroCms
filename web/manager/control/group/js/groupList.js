...
var SiteGroupBean = new Bean("com.cicro.wcm.bean.control.SiteGroupBean",true);
...
{
	top.OpenModalWindow("维护站群","/manager/control/group/group_add.jsp?parentID="+sgroup_id,500,300);
}

function openUpdateGroupPage()
{
	var selectIDS = table.getSelecteCheckboxValue("sgroup_id");
	top.OpenModalWindow("维护站群","/manager/control/group/group_add.jsp?parentID="+sgroup_id+"&sgroup_id="+selectIDS,500,300);
}

function openViewPage(s_id)
{
	top.OpenModalWindow("查看站群","/manager/control/group/group_view.jsp?sgroup_id="+s_id,500,300); 
}
...
function addGroup(){
	...
	if(SiteGroupRPC.insertSiteGroup(SiteGroupBean))
	{
		...
		top.getCurrentFrameObj().changeMemuListTable(SiteGroupBean.parent_id);
		top.getCurrentFrameObj().insertMenuTree(SiteGroupBean.sgroup_id,SiteGroupBean.sgroup_name);
		//top.getCurrentFrameObj().treeNodeSelected(SiteGroupBean.parent_id);
...
function deleteGroup()
{
	...
	if(SiteGroupRPC.getSGroupSortByID(selectIDS)!='0'){
		top.msgWargin("该站群下面有节点，不能删除");
		return;
	}
	
	if(SiteGroupRPC.deleteSiteGroup(selectIDS))
	{
		top.msgAlert("站群信息"+WCMLang.Delete_success);
		top.getCurrentFrameObj().changeMemuListTable(sgroup_id);
		top.getCurrentFrameObj().deleteTreeNode(selectIDS);
	}else
	{ 
		top.msgWargin("站群信息"+WCMLang.Delete_fail);
	}
}
...
function updateGroup(){
	...
	if(SiteGroupRPC.updateSiteGroup(SiteGroupBean))
	{
		top.msgAlert("站群信息"+WCMLang.Add_success);
		top.getCurrentFrameObj().changeMemuListTable(SiteGroupBean.parent_id);
		top.getCurrentFrameObj().updateMenuTree(SiteGroupBean.sgroup_id,SiteGroupBean.sgroup_name);
		top.CloseModalWindow();
	}
	else
	{
