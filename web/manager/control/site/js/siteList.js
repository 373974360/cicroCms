...
var SiteBean = new Bean("com.cicro.wcm.bean.control.SiteBean",true);
...
function showList(){			
	...
	table.getCol("domain_col").each(function(i){	
		if(i>0)
		{			
			$(this).html('<a href="javascript:top.addTab(true,\'/manager/control/site/domainList.jsp?site_id='+beanList.get(i-1).site_id+'\',\'域名管理\');">域名管理</a>');			
		}
	});
...
{  
	...
	if(true)
	{
		top.addTab(true,'/manager/control/site/site_add.jsp?parentID='+site_id+"&tab_index="+top.curTabIndex,'维护站点');
	}else
	{
		top.msgWargin("您所购站点数已创建完成，如需要增加站点，请联系厂商");
	}
}

function openUpdateSitePage()
{  
	//top.OpenModalWindow("维护站点","/manager/control/site/site_add.jsp?parentID="+site_id,500,360);
	var selectIDS = table.getSelecteCheckboxValue("site_id");
	top.addTab(true,'/manager/control/site/site_add.jsp?site_id='+selectIDS+'&parentID='+site_id+"&tab_index="+top.curTabIndex,'维护站点');
}

function openViewPage(s_id)
{  
	top.addTab(true,'/manager/control/site/site_view.jsp?site_id='+s_id+"&tab_index="+top.curTabIndex,'查看站点');
}
...
function addSite(){
	...
	{
		top.msgAlert("站点信息"+WCMLang.Add_success);
		top.getCurrentFrameObj(tab_index).changeMemuListTable(SiteBean.parent_id);
		top.getCurrentFrameObj(tab_index).insertMenuTree(SiteBean.site_id,SiteBean.site_name);
		top.tab_colseOnclick(top.curTabIndex);
	}
	else
	{
		top.msgWargin("站点信息"+WCMLang.Add_fail);
	}
...
function deleteSite(){
	...
	if(SiteRPC.getSiteSortByID(selectIDS)!='0'){
		top.msgWargin("该站点下面有站点，不能删除");
		return;
	}
	
	if(SiteRPC.deleteSite(selectIDS)){
		top.msgAlert("站点信息"+WCMLang.Delete_success);
		changeMemuListTable(site_id);
		deleteTreeNode(selectIDS); 
	}else
	{
		top.msgWargin("站点信息"+WCMLang.Delete_fail);
	}
}
...
function funPause()
{
	...
	if(SiteRPC.updateSiteStatus(selectIDS,"1"))
	{
		...
		top.getCurrentFrameObj().changeMemuListTable(site_id);
		//showMenuTree(); 
	}else
	{
...
function updateSite(){
	...
	*/
	...
	if(SiteRPC.updateSite(SiteBean))
	{
		top.msgAlert("站点信息"+WCMLang.Add_success);
		top.getCurrentFrameObj(tab_index).changeMemuListTable(SiteBean.parent_id);
		top.getCurrentFrameObj(tab_index).updateMenuTree(SiteBean.site_id,SiteBean.site_name);
		top.tab_colseOnclick(top.curTabIndex);
	}
	else
	{
...
function saveUser(user_ids){
	...
	{  
		top.msgAlert("站点管理员"+WCMLang.Add_success);
		table.unChekcbox();
	}
	else
	{
		top.msgWargin("站点管理员"+WCMLang.Add_fail);
	}
}
