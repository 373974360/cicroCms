...
var SiteDomainBean = new Bean("com.cicro.wcm.bean.control.SiteDomainBean",true);
...
{  
	top.OpenModalWindow("维护域名","/manager/control/site/domain_add.jsp?site_id="+site_id,460,130);
}
...
{  	
	...
	top.OpenModalWindow("维护域名","/manager/control/site/domain_add.jsp?site_id="+site_id+"&domain_id="+selectIDS,460,130);
}
...
function insertSiteDomain(){
	...
	{
		top.msgAlert("域名"+WCMLang.Add_success);
		top.getCurrentFrameObj().reloadDomainList();
		top.CloseModalWindow();
	}
	else
	{
		top.msgWargin("域名"+WCMLang.Add_fail);
	}
...
function setDefault()
{
	...
	{
		top.msgAlert("默认域名"+WCMLang.Set_success);
		reloadDomainList();
	}
	else
	{
		top.msgWargin("默认域名"+WCMLang.Set_fail);
	}
}
...
function deleteSiteDomain()
{
	...
	for(var i=0;i<list.size();i++)
	{
		...
		{
			top.msgWargin("主域名不能删除,请重新选择删除项");
			return
		}
	...
	{
		top.msgAlert("域名"+WCMLang.Delete_success);
		reloadDomainList();
	}
	else
	{
		top.msgWargin("域名"+WCMLang.Delete_fail);
	}
}
