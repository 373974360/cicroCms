var bean = new Bean("com.cicro.project.searchkeys.SearchKey",true);
...
function showList(){	
	...
	updateType = false;
	setTimeout(function(){
		top.jsonrpc.SearchKeyRPC.getSearchKeysList(showListResult,con_m);//第一个参数为站点ID，暂时默认为空	
	},12);
}
...
function showTurnPage(){
	setTimeout(function(){
		top.jsonrpc.SearchKeyRPC.getSearchKeysListCount(showTurnPageResult,con_m);
	},12);
}
...
{
	top.OpenModalWindow("新建关键词","/manager/project/searchkeys/searchkeyList_add.jsp?site_id="+site_id,460,245);
}
...
function openUpdatePage2(selectIDS){
	//top.addTab(true,'/manager/searchkeys/searchkeyList_add.jsp?id='+selectIDS+'&tab_index='+top.curTabIndex,'修改');
	top.OpenModalWindow("修改关键词","/manager/project/searchkeys/searchkeyList_add.jsp?site_id="+site_id+"&id="+selectIDS,460,245);
	//top.OpenModalWindow("修改","/manager/nongye/member/member_card_add.jsp?card_id="+selectIDS,900,640);
}
...
function deleteData2(m_id)
{
	...
	top.jsonrpc.SearchKeyRPC.deleteSearchKeys(deleteDataResult,m_id);
}
