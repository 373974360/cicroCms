...
var collRuleBean = new Bean("com.cicro.wcm.dataCollection.bean.CollRuleBean",true);
var CategoryRPC = jsonrpc.CategoryRPC;

//添加url地址
function addStartURL(hand_name)
{
	top.OpenModalWindow("添加采集网址","/manager/dataCollection/addURL.jsp?hand_name="+hand_name,800,500);
}
...
function UpdateStartURL()
{
	if(selectRow)
	{
		...
		top.OpenModalWindow("修改采集网址","/manager/dataCollection/updateURL.jsp?hand_name=saveStartURL"+"&urlStr="+urlstr+"&rowIndex="+selRowIndex,800,500);
	}else{
...
//	collRuleBean.timeFormatType= $("#timeFormatType").val();
	...
	if($("#cat_name").val()!="--请选择栏目--"){
		collRuleBean.cat_name = $("#cat_name").val();
		collRuleBean.cat_id = $("#catidhidden").val();
	}else{
		top.msgAlert("请选择入库信息!");
	}
	
	var s = $("#coll_interval").attr("checked");
	if(s){
...
function checkCollUrl()
{
	...
	top.OpenModalWindow("测试采集地址","/manager/dataCollection/checkCollURL.jsp?collURL="+collURL+"&ListStart="+listUrlstart+"&ListEnd="+listUrlend+"&encode="+encode,800,550);
}
...
function initCategoryTree()
{
	$('#leftMenuTree').tree({
		onClick:function(node){			 
			changeCategoryListTable(node.id,node.text);           
		}
	});
}

function changeCategoryListTable(Category_id,Category_name)
{
	$("#cat_name").val(Category_name);
	$("#catidhidden").val(Category_id);
}