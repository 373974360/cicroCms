...
{
	top.addTab(true,"/manager/dataCollection/updateCollInfo.jsp?tab_index="+top.curTabIndex+"&info_id="+infoID,"修改采集信息");
}
...
function pubCollDataInfotoCat()
{
	...
	for(var i=0;i<artBeanList.size();i++)
	{
		...
		bean.info_status = info_status;
		bean.info_content = artBeanList.get(i).art_content;
		bean.page_count = "1";
		bean.cat_id = artBeanList.get(i).cat_id;
		bean.title = artBeanList.get(i).art_title.replace(/\"/g,"＂");;
		bean.source = artBeanList.get(i).art_source;
		bean.info_keywords = artBeanList.get(i).art_keyWords;
		bean.app_id = "cms";
		bean.site_id = site_id;
		bean.model_id= "11";
		bean.released_dtime = artBeanList.get(i).art_pubTime;
		bean.is_am_tupage = 0;
		bean.input_user = top.LoginUserBean.user_id;
		bean.weight = 60;
		if(ModelUtilRPC.insert(bean,"article"))
		{	
