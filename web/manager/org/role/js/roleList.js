...
var RoleBean = new Bean("com.cicro.wcm.bean.org.role.RoleBean",true);
var RoleUserBean = new Bean("com.cicro.wcm.bean.org.role.RoleUserBean",true);
var RoleUGroupBean = new Bean("com.cicro.wcm.bean.org.role.RoleUGroupBean",true);
...
function showList(){	
	...
	table.getCol("action_col").each(function(i){
		if(i>0)
		{
			...
			if(app_id != 'zwgk')
			{
				str += '<a href="javascript:openReleUserHandl('+(i-1)+')">关联用户</a>&#160;&#160;';
			}
			/*
			if(beanList.get(i-1).app_id != app_id)
			{
...
function openViewRolePage(r_id)
{
	...
	if(app_id == "system"|| app_id == "control")
		height = 360;
	top.OpenModalWindow("维护角色","/manager/org/role/role_add.jsp?app="+app_id+"&role_id="+r_id,450,height);
}
...
function openAddRolePage()
{
	...
	if(app_id == "system" || app_id == "control")
		height = 360;
	top.OpenModalWindow("维护角色","/manager/org/role/role_add.jsp?app="+app_id+"&site_id="+site_id,450,height);	
}
...
function openUpdateRolePage()
{
	...
	}else
	{
		...
		top.OpenModalWindow("维护角色","/manager/org/role/role_add.jsp?app="+app_id+"&role_id="+selectIDS,450,height);
	}
}
