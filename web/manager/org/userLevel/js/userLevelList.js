...
var UserLevelBean = new Bean("com.cicro.wcm.bean.org.user.UserLevelBean",true);
...
function showList()
{
	...
	table.getCol("userlevel_name").each(function(i){
		if(i>0)
		{
			$(this).html('<a href="javascript:top.OpenModalWindow(\'用户级别信息\',\'/manager/org/userLevel/userLevel_view.jsp?userlevel_id='+beanList.get(i-1).userlevel_id +'\',450,245)">'+beanList.get(i-1).userlevel_name+'</a>');	
		}
	});
...
{
	top.OpenModalWindow("新建用户级别","/manager/org/userLevel/userLevel_add.jsp?type=add&userlevel_id=",460,245);
}
...
function updateLevel()
{
	...
	top.OpenModalWindow("修改用户级别","/manager/org/userLevel/userLevel_add.jsp?type=update&userlevel_id="+userlevel_id,430,260);
}
