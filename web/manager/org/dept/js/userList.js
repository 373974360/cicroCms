...
var UserBean = new Bean("com.cicro.wcm.bean.org.user.UserBean",true);
var UserRegisterBean = new Bean("com.cicro.wcm.bean.org.user.UserRegisterBean",true);
...
function showUserList(){			
	...
	user_table.getCol("user_realname").each(function(i){
		if(i>0)
		{
			$(this).html('<a href="javascript:top.addTab(true,\'/manager/org/dept/user_view.jsp?user_id='+user_beanList.get(i-1).user_id+'\',\'用户信息\')">'+user_beanList.get(i-1).user_realname+'</a>');
		}
	});
...
function checkUserInfo()
{
	...
	if($("#isAddReg").attr("checked") == true || $("#isAddReg").attr("checked") == "checked")
	{
		isAddReg = true;
...
function addUser()
{
	...
	bean.resume = KE.html("resume");
	bean.summary = KE.html("summary");
...
{	
	var selectIDS = user_table.getSelecteCheckboxValue("user_id");
	top.OpenModalWindow("维护帐号","/manager/org/dept/registerUser_update.jsp?user_id="+selectIDS,450,210);	
}
