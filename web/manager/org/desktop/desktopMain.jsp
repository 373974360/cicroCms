...
$(document).ready(function(){
	...
	if(apps.indexOf("zwgk") > -1)
	{
		initSiteListHandel("zwgk","zwgk_table");
		$("#zwgk_box").show();
	}
	init_input();
...
function initSiteListHandel(app_name,table_name)
{
	var list;
	if(app_name == "cms")
		list = UserManRPC.getAllUserSiteObjList(user_id,app_name);
	if(app_name == "zwgk")
		list = UserManRPC.getAllUserGKNodeObjList(user_id,app_name);
...
{
	var DeskTopBean = new Bean("com.cicro.wcm.bean.org.desktop.DeskTopBean",true);
...
<div class="sq_box hidden" id="sq_box">
	...
	<table id="sq_table" class="table_view" border="0" cellpadding="0" cellspacing="0">
		<tbody>
			<tr>
				<td><ul><li><input type="checkbox" value="0"><label>首接件</label></li><li><input type="checkbox" value="1"><label>转办件</label></li></ul></td>
			</tr>
			<tr>
				<td><ul><li><input type="checkbox" value="2"><label>待审件</label></li><li><input type="checkbox" value="3"><label>延期待审</label></li></ul></td>
			</tr>
			<tr>				
				<td><span class="pointer" onclick="selectAll('sq_table')">全选</span>&nbsp;|&nbsp;<span class="pointer" onclick="concelAll('sq_table')">反选</span></td>
			</tr>
		</tbody>
	</table>
	</div>
...
<span class="blank6"></span>
<!--相关信件-->
<div class="sq_box hidden"  id="zwgk_box">
	<div class="sq_title_box" >
		<div class="sq_title sq_title_minus">信息公开待审信息</div>
		<div class="sq_title_right">点击闭合</div>
	</div>
	<div class="sq_box_content">
