...
var GroupBean = new Bean("com.cicro.wcm.bean.org.group.GroupBean",true);
...
function showList()
{
	...
	table.getCol("group_name").each(function(i){
		if(i>0)
		{
			$(this).html('<a href="javascript:top.addTab(true,\'/manager/org/group/group_view.jsp?appID='+appId+'&site_id='+siteId+'&groupId='+beanList.get(i-1).group_id+'\',\'用户组信息\')">'+beanList.get(i-1).group_name+'</a>');	
		}
	});
...
function addGroup()
{
	top.OpenModalWindow("用户组添加","/manager/org/group/group_add.jsp?appId="+appId+"&site_id="+siteId,420,220);
}
...
{
	top.OpenModalWindow("查看用户组信息","/manager/org/group/group_view.jsp?groupId="+groupId,450,190);	
}
...
function updateGroup()
{
	...
	top.OpenModalWindow("用户组修改","/manager/org/group/group_update.jsp?groupId="+groupId,420,220);
}
