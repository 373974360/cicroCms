...
function setBIdsHtml(){
		...
		top.OpenModalWindow("设置业务","/manager/appeal/count/aim/businessTree.jsp",330,450);
}
...
function funOK(){
    ...
    //var countType = $(":radio[name='countType'][checked]").val();
    var countType ="dept";
	
	if(countType == 'dept'){
		href_str = "letterHandleR.jsp?s="+timeS+"&e="+timeE+"&b_ids="+b_ids;
		$("#countList").attr("src",href_str);
		$("#iframeTable").show();
		$("#line2h").show();
		$("#buttonTable").show();
	}else if(countType == 'user')
	{
		href_str = "letterHandleRUsers.jsp?s="+timeS+"&e="+timeE+"&b_ids="+b_ids;
		$("#countList").attr("src",href_str);
		$("#iframeTable").show();
		$("#line2h").show();
		$("#buttonTable").show();
