	...
	{		
		...
		if(defaultBean)
		{
			...
			if(defaultBean.sq_reply != "")
			{
			  ...
			  var dept_name = jsonrpc.CpDeptRPC.getCpDeptName(defaultBean.do_dept);
			  var str = "</tr>";
				str += '<tr class="trList">';
...
{
	top.OpenModalWindow("内容编辑","/manager/appeal/sq/update_process.jsp?pro_id="+pro_id+"&top_index="+top.curTabIndex,650,340);
}
...
{	
	...
	{		
		...
		str += '<input type="text" readonly=true value="" id="dept_idTemp" name="dept_idTemp"/><input id="btn3" name="btn3" type="button" value="选择部门" onclick="openSelectCpDeptPage(\'选择部门\',\'setCpDept\')" class="btn x4 btn-over">';
	}
	$("#dept_list").html(str);
}
...
function setCpDept(id,name){
	var str = '<li style="width:120px;overflow:hidden;display:block;height:20px;padding-right:5px" title="'+name+'"><input type="radio" id="do_dept" name="do_dept" value="'+id+'"><label>'+name+'</label></li>'+$("#dept_list").html();
	$("#dept_list").html(str);
   $("#dept_idTemp").val(name);
   $(":radio[name='do_dept'][value='"+id+"']").attr("checked",true);
}
//李苏培加-----end
...
{
	top.addTab(true,"/manager/appeal/sq/sq_panchong.jsp?top_index="+top.curTabIndex,"信件判重");
}
...
function updateSQ()
{		
	top.addTab(true,"/manager/appeal/sq/sq_update.jsp?sq_id="+sq_id+"&top_index="+top.curTabIndex,"信件编辑");
}

function openPrintPage()
{
	window.open("/manager/appeal/sq/print.jsp?st=m&sq_id="+sq_id+"&model_id="+defaultBean.model_id);
}
function creatWord()
{
   var url="/manager/appeal/sq/printWord.jsp?st=m&sq_id="+sq_id+"&model_id="+defaultBean.model_id;  
   //window.location.href=url;
   window.open(url);
}


function openChangeDtimePage()
{	
	top.OpenModalWindow("修改回复时间","/manager/appeal/sq/changeOverDtime.jsp?sq_id="+sq_id+"&top_index="+top.curTabIndex,315,170);	
}
...
<div class="sq_box">
	...
	<div class="sq_box_content">
		<table id="infoList" class="table_dolist" border="0" cellpadding="0" cellspacing="0">
			<thead>
				<tr>
					<td width="25" align="center">步骤</td>
...
<table class="table_option" border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td align="left" valign="middle" style="text-indent:10px;">
			...
			<input id="btn446" name="btn446" class="hidden" type="button" onclick="openChangeDtimePage()" value="修改回复时间" />
		</td>
	</tr>
</table>
<span class="blank6"></span>
