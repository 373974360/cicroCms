...
var site_id = SiteRPC.getSiteAppBean(app_id);
...
$(document).ready(function(){
	...
	var kcat_names = InfoRelatedKcatRPC.getRelatedKcatNames(sq_id);
	if(kcat_names != "")
	{
...
function updateSQ()
{
	...
	bean.sq_content2 = KE.html("sq_content2");
	...
	if(SQRPC.updateSQ(bean,tag_ids))
	{
		...
		//知识库标签入库 -- start  add by zhangqiang 2013-09-10
		insertInfoRelatedKcat(bean);
		//知识库标签入库 --end
...
function insertInfoRelatedKcat(bean)
{
	...
	if(kcat_name !="" && kcat_id !="")
	{
		...
		kcat_map.put("info_id",bean.sq_id+"");
		kcat_map.put("site_id","appeal");
		kcat_map.put("app_id","appeal");
...
function showSelectKDiv(input_name,div_name,height,tree_div_name)
	{
		...
		$("input[id='"+input_name+"']").click(function(event){
			$("div.select_div2").hide();//先关闭其它的div
...
{ 
	kcat_jdata = eval(KnowledgeRPC.getJSONTreeStr(site_id,""));
	...
	<tbody>
		...
		<tr>
			...
			<td>
			   ...
			   <div id="kcat_tree_div" class="select_div2 tip hidden border_color" style="width:200px;height:300px;overflow:hidden;border:1px #7f9db9 solid;" >
					<div id="leftMenuBox_k">
					     <div id="leftMenu_k" class="contentBox6 textLeft" style="overflow:auto">
							<ul id="leftMenuTree_k" class="easyui-tree" animate="true" style="width:200px;height:300px;"></ul>
						 </div>
				    </div>
			    </div>
			 </td>
		...
		<tr>
			<th>信件内容 ：</th>
			<td>
				<textarea id="sq_content2" name="sq_content2" style="width:620px;height:200px"></textarea>
			</td>
		</tr>
		<tr>
			<th>权重：</th>
