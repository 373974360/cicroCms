...
var SQBean = new Bean("com.cicro.wcm.bean.appeal.sq.SQBean",true);
var ProcessBean = new Bean("com.cicro.wcm.bean.appeal.sq.ProcessBean",true);
var SQCustom = new Bean("com.cicro.wcm.bean.appeal.sq.SQCustom",true);
...
{
	top.addTab(true,"/manager/appeal/sq/sq_add.jsp?sq_id="+s_id+"&top_index="+top.curTabIndex,"信件管理");	
	//window.location.href = "/manager/appeal/sq/sq_add.jsp?sq_id="+s_id;
}

function openSQManager2(s_id)
{
	top.addTab(true,"/manager/appeal/sq/sq_add2.jsp?sq_id="+s_id+"&top_index="+top.curTabIndex,"信件管理");	
	//window.location.href = "/manager/appeal/sq/sq_add.jsp?sq_id="+s_id;
}
...
function setChongfu()
{	
	initial_sq_id = $(":radio[id='initial_sq_id'][checked=true]").val();	
	 
	if($(":radio[id='initial_sq_id'][checked=true]").length == 0)
	{
...
function insertProcess(pro_type)
{
	...
	switch(pro_type)
	{
		case 0:sqMap.put("sq_status",1+"");//受理时，流程处理状态改为处理中	
				sqMap.put("is_back",0);//回退标识 0：正常
			   break;
		case 1:if(model_bean.wf_id > 0 && is_admin == false)////判断是否需要审核(流程ID大于0，判断是否是管理员，如果是不要审核，一步到位)
				{					
					//根据人员ID得到下一步的步骤ID
				...
				sqMap.put("is_back",0);//回退标识 0：正常
				sqMap.put("publish_status",$(":radio[id='publish_status'][checked=true]").val()+"");//发布标识 
				sqMap.put("publish_dtime","");
				sqMap.put("is_open",$(":radio[id='is_open_r'][checked=true]").val());
				sqMap.put("sq_reply",pro_note);
				break;
		case 2:sqMap.put("sq_status",1+"");//转办时，流程处理状态改为处理中				
			   sqMap.put("do_dept",$(":radio[id='do_dept'][checked=true]").val());//设置转办部门 			   
			   ...
			   break;
		case 3:sqMap.put("sq_status",1+"");//交办时，流程处理状态改为处理中
			   sqMap.put("do_dept",$(":radio[id='do_dept'][checked=true]").val());
			   ...
			   break;
		case 4:sqMap.put("sq_status",1+"");//呈办时，流程处理状态改为处理中
			   sqMap.put("do_dept",$(":radio[id='do_dept'][checked=true]").val());
			   ...
			   break;
		case 5:sqMap.put("sq_flag",1);//1：重复信件
				...
				sqMap.put("sq_reply",pro_note);
			   break;
		case 6:sqMap.put("sq_flag",-1);//置为无效状态	
				sqMap.put("sq_status",3+"");//办结
				sqMap.put("step_id",100);//流程状态码 100：终审通过
				sqMap.put("is_back",0);//回退标识 0：正常
				break;
		case 7:sqMap.put("sq_flag",2);//不予受理时，信件标识改为2
			   if(model_bean.wf_id > 0 && is_admin == false)//判断是否需要审核(还要判断是否是管理员，如果是不要审核，一步到位)，如果要审核，流程处理状态　改为2：待审核　否则改为3：已办结
				{
					var step_id = jsonrpc.WorkFlowRPC.getNextStepIDByUserID(model_bean.wf_id,top.LoginUserBean.user_id,"appeal","");
					if(step_id == 100)
		...
		case 100://内容审核　根据人员ID得到下一步的步骤ID
				...
				{
					if($(":radio[id='auto_status'][checked=true]").val() == 12)
					{
						//审核打回
				...
				{//以下是管理员和最后一级审核员的操作					
					if($(":radio[id='auto_status'][checked=true]").val() == 12)//不通过
					{
						sqMap.put("is_back",1);//退回标识
						sqMap.put("sq_status",1+"");//处理中
						sqMap.put("sq_flag",0);//信件标识 信件标识都改为正常
				...
				sqMap.put("is_open",$(":radio[id='is_open_r'][checked=true]").val());
				sqMap.put("publish_status",$(":radio[id='publish_status'][checked=true]").val()+"");
				sqMap.put("publish_dtime","");
			   break;
		case 8:sqMap.put("limit_flag",2);//延时申请标识 2：申请延时
		       sqMap.put("is_back",0);
			   break;
		case 101:if($(":radio[id='auto_status'][checked=true]").val() == 9) //延期审核
				 {//审核通过
				 ...
				 break;
		case 13:sqMap.put("supervise_flag",$(":radio[id=supervise_flag][checked=true]").val());//督办状态				
				break;
		case 102:sqMap.put("publish_status",$(":radio[id='publish_status'][checked=true]").val()+"");//发布标识				 
				 sqMap.put("is_open",$(":radio[id='is_open_r'][checked=true]").val());
				 sqMap.put("sq_title2",$("#sq_title2_r").val());
				 sqMap.put("sq_content2",KE.html("sq_content2_r"));
				 sqMap.put("sq_reply",pro_note);
	...
	if(pro_type != 102)//102为发布，发布不写过程表
	{
		...
		{
			bean.pro_type = $(":radio[id='auto_status'][checked=true]").val();
		}else
	...
	var isPub = $(":radio[id='publish_status'][checked=true]").val()+"";

	if((pro_type == 1 && isPub == "1") || pro_type == 102)
	{
		var no = defaultBean.sq_phone;
		if(no != null && no != "")
		{
			$.ajax({  
			  type: "GET", // 用POST方式传输  
				dataType: "text", // 数据格式:JSON  
				url: "/servlet/SendMsg", // 目标地址  
				data: "no=" + no + "&sq_id=" + sq_id,
...
function initUPLoad()
{
	var sizeLimit = SQRPC.getAppealFileSize();
	 ...
	 $("#uploadify").uploadify( {//初始化函数
		...
		'fileDesc' :'jpg,png,txt,doc,docx,rar,zip',//上传文件类型说明
