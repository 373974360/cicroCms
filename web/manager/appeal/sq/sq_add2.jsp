...
function getProcessList()
{
	...
	if(pl != null && pl.size() > 0)
	{
		for(var i=0;i<pl.size();i++)
		{
			...
			str += '<td>'+pl.get(i).pro_note.replace(/\n/,"</br>")+'';
			str += '</td>';
			str += '</tr>';
			str += '<tr>';
	...
	{		
		for(var i=0;i<dept_list.size();i++)
		{
			if(current_dept_id != dept_list.get(i).dept_id)
				str += '<li><input type="radio" id="do_dept" name="do_dept" value="'+dept_list.get(i).dept_id+'">'+dept_list.get(i).dept_name+'<label></label></li>';
		}		
	}
	$("#dept_list").html(str);
}
	...
	<table id="" class="table_view" border="0" cellpadding="0" cellspacing="0">
		<tbody>
			...
			<tr>
				<th>&nbsp;</th>
				<td>					
					<input id="btn180" name="btn3" type="button" onclick="openPrintPage()" value="信件打印" />
				</td>
			</tr>
		</tbody>
	</table>
