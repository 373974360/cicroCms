...
var ModelBean = new Bean("com.cicro.wcm.bean.appeal.model.ModelBean",true);
...
function openViewModelPage(r_id)
{
	...
	if(app_id == "system")
		height = 360;
	top.OpenModalWindow("查看业务","/manager/appeal/model/model_view.jsp?app="+app_id+"&model_id="+r_id,450,height);	
}

//打开添加窗口
function openAddModelPage()
{
	window.location.href = "/manager/appeal/model/model_add.jsp";	
}
...
function openUpdateModelPage(m_ids)
{
	...
	if(m_ids == "" || m_ids == null)
		...
		selectIDS = m_ids;
	window.location.href = "/manager/appeal/model/model_add.jsp?model_id="+selectIDS;
}
...
{
	top.OpenModalWindow("导出数据","/manager/appeal/model/exportData.jsp?model_id="+modelId+"",330,220);
}
...
{
	top.OpenModalWindow("批量不公开","/manager/appeal/model/bachIsNotOpen.jsp?model_id="+model_ids+"",400,150);
}
