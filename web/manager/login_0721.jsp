...
<%
	...
	{
		com.cicro.wcm.bean.org.user.UserBean user = com.cicro.wcm.services.org.user.UserRegisterManager.getUserBeanByUname(user_name);
		String today = com.cicro.util.DateUtil.getCurrentDate();
		if(user != null)
		{
			if(user.getGraduation_time() != null && !"".equals(getGraduation_time))
			{
				if(today.equals(user.getGraduation_time()))
				{
					if(user.getAge() >= 5)
					{
						out.println("<script>alert(\"登录密码错误超过5次，无法登录！\");top.changeCreateImage();</script>");		
						return;
					}
				}
			}
		...
		else
		{
			if(!"0".equals(res))		
			{
				...
				switch(msg)
				{
					...
					case 3:
						out.println("<script>alert(\"用户名不正确，错误超过5次将无法登录\");top.changeCreateImage();</script>");
						if(user != null)
						{
							if(user.getGraduation_time() != null && !"".equals(getGraduation_time))
							{
								if(today.equals(user.getGraduation_time()))
								{
									...
									com.cicro.wcm.services.org.user.UserManager.updateUser(user,null);
								}
								else
								{
									user.setGraduation_time(today);
									user.setAge(1);
									com.cicro.wcm.services.org.user.UserManager.updateUser(user,null);
								}
							}
						...
						break;//用户名不正确
					case 4:
						out.println("<script>alert(\"用户名密码不正确，错误超过5次将无法登录\");top.changeCreateImage();</script>");
						if(user != null)
						{
							if(user.getGraduation_time() != null && !"".equals(getGraduation_time))
							{
