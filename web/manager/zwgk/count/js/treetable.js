...
function createTableAllSite()
{
	...
	if(beanList.size() != 0) {
		...
		treeHtmls += "<tr><td>信息合计</td>";
		treeHtmls+="<td>"+all_info_count +"</td>";
		treeHtmls+="<td>"+all_z_count +"</td>";
		treeHtmls+="<td>"+all_y_count +"</td>";
		treeHtmls+="<td>"+all_b_count +"</td>";
		treeHtmls+="<td>"+all_pic_count +"</td>";
		treeHtmls+="<td>&nbsp;</td></tr>";
...
function setHandlListAllSite(bean, index)
{
	...
	if(bean)
	{
		...
		str+="<td>"+bean.pic_count +"</td>";
		str+="<td><a href='javascript:openPieChart("+index+")'>"+"查看"+"</a></td>";
		all_info_count += bean.info_count;
		all_z_count += bean.z_count;
		all_y_count += bean.y_count;
		all_b_count += bean.b_count;
		all_pic_count += bean.pic_count;
	}
	return str;
}
...
{
	top.addTab(true ,"/manager/zwgk/count/gkCountList.jsp?start_day="+start_day+"&end_day="+end_day+"&site_id="
...
function openPieChart(index) {
	...
	top.OpenModalWindow("饼状图","/manager/zwgk/count/chart.html",475,470);
