...
// 打开选择节点的弹出窗口
function openPage() {
	top.OpenModalWindow("选择公开节点","/manager/zwgk/count/nodeSelected.jsp",725,525);
}
...
// 点击取得全部节点触发
function setAllNode() {
	is_all_node = $("#setAll").attr("checked");
	if(is_all_node) {
		$("#cat_tree").hide();
		$("#openPage").hide();
		// getAllNode_ids();
...
{
	$("#countList").empty();
	beanList = jsonrpc.GKCountRPC.getYSQStateCntByNode(mp); //  取得依申请公开申请单状态统计
...
{			
	var treeHtml = "";
	var type_calss= "";
	treeHtml+="<tr> <td><span class='file'>"+bean.put_dtime+"</span></td>";
...
function createPieData(bean) {
	...
	var ret =  '{ "title": { "text": "'+node_name_site + bean.put_dtime +'详细统计信息"}, ' +
