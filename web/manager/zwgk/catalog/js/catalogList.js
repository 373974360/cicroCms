...
var AppCatalogBean = new Bean("com.cicro.wcm.bean.zwgk.appcatalog.AppCatalogBean",true);
...
{
	top.addTab(true,"/manager/zwgk/catalog/catalogMain.jsp?top_index="+top.curTabIndex+"&parentID="+c_id,"目录管理");
}

function openAddCatalogPage()
{
	top.addTab(true,"/manager/zwgk/catalog/catalog_add.jsp?top_index="+top.curTabIndex+"&parentID="+cata_id,"维护目录");
}
//打开规则选择窗口
function openSelectRegulation(c_id)
{
	top.OpenModalWindow("信息汇聚规则设置","/manager/zwgk/catalog/set_regulation.jsp?cata_id="+c_id,860,600);
}
...
function openUpdateCatalogPage(cid)
{
	...
	top.addTab(true,"/manager/zwgk/catalog/catalog_add.jsp?top_index="+top.curTabIndex+"&parentID="+cata_id+"&cata_id="+id,"维护目录");
}
