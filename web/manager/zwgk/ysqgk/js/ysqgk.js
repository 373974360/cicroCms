...
var YsqgkConfigBean = new Bean("com.cicro.wcm.bean.zwgk.ysqgk.YsqgkConfigBean",true);
var YsqgkBean = new Bean("com.cicro.wcm.bean.zwgk.ysqgk.YsqgkBean",true);
var YsqgkListBean = new Bean("com.cicro.wcm.bean.zwgk.ysqgk.YsqgkListBean",true);
...
{
	 var str ="";
     $("[name='"+name+"'][checked]").each(function(){str+=$(this).val()+",";})
     if(str.endsWith(",")){
...
{		
	...
	ysqgk_bean.offer_type = getSelectedCheckBox('offer_type');
	ysqgk_bean.get_method = getSelectedCheckBox('get_method');
	
	if(YsqgkRPC.insertYsqgkInfo(ysqgk_bean))
	{
		top.msgAlert("依申请公开信息"+WCMLang.Add_success+"!");
		window.location.href = "/manager/zwgk/ysqgk/operate/ysqgk_list.jsp?app_id=zwgk&site_id="+node_id;
	}
	else
	{
...
function initTable(node_id)
{
	...
	colsList.add(setTitleClos("put_dtime","提交时间","200px","","",""));
	colsList.add(setTitleClos("publish_state","发布状态","80px","","",""));
	colsList.add(setTitleClos("is_third","征询第三方","80px","","",""));
	colsList.add(setTitleClos("timeout_flag","是否超时","70px","","",""));
	colsList.add(setTitleClos("is_extend","是否暂缓","80px","","",""));
...
function SubmitInfo(n,node_id){	
	addYsqgkInfo(node_id);
	//insertProcess(n,node_id,YsqgkBean.ysq_id);
}
...
{
	top.addTab(true,"/manager/zwgk/ysqgk/operate/ysqgk_info.jsp?ysq_id="+ysq_id+"&node_id="+node_id,"依申请公开");
}

function openReplyInfoPage(ysq_id,node_id)
{
	top.addTab(true,"/manager/zwgk/ysqgk/operate/ysqgk_Replayinfo.jsp?ysq_id="+ysq_id+"&node_id="+node_id,"依申请公开");
}

function openAddPage(node_id)
{
	top.addTab(true,"/manager/zwgk/ysqgk/operate/ysqgk_infoAdd.jsp?node_id="+node_id,"依申请公开");
}
//高级搜索页面
function openHighSearchPage(node_id)
{
	top.OpenModalWindow("高级搜索","/manager/zwgk/ysqgk/operate/hightSearch.jsp?node_id="+node_id,510,500);
}

function openUpdatePage(node_id)
{
	var ysq_id = table.getSelecteCheckboxValue("ysq_id");
	top.addTab(true,"/manager/zwgk/ysqgk/operate/ysqgk_infoUpdate.jsp?ysq_id="+ysq_id+"&node_id="+node_id,"依申请公开");	
}
...
function setSelectedCommonLang(c_type)
{
	...
	if(c_type != "")
	{
		var content = lang_map.get(c_type);		
		    content = content.replace(/\{name\}/ig,YsqgkBean.name).replace(/\{ysq_code\}/ig,YsqgkBean.ysq_code).replace(/\{put_dtime\}/ig,YsqgkBean.put_dtime).replace(/\{now_time\}/ig,getCurrentDateTime()).replace(/\{node_name\}/ig,YsqgkBean.node_name).replace(/\{email\}/ig,YsqgkBean.email);
	}
...
function insertAddress(){
	...
	if(GKNodeBean !="" || GKNodeBean != null)
	{
		var address = GKNodeBean.address;
		if(address !=""){
			tmp += "办公地址:"+address+"; ";
		}
		var postcode = GKNodeBean.postcode;
		if(postcode !=""){
			tmp += "邮政编码:"+postcode+"; ";
		}
		var tel = GKNodeBean.tel;
		if(tel !=""){
			tmp += "办公电话:"+tel+"; ";		
		}
		var fax = GKNodeBean.fax;
		if(fax !=""){
			tmp += "传真:"+fax+"; ";		
		}
		var email = GKNodeBean.email;
		if(email !=""){
			tmp += "电子邮件:"+email+"; ";		
		}
		if(tmp == "" || tmp == null)
		{
			...
			KE.util.focus("deal_content");
		    KE.util.selection("deal_content");
		    KE.util.insertHtml("deal_content",tmp);
		}
	}
}
