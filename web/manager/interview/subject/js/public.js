...
function fnAddSubject(main_type)
{
	...
	top.addTab(true,"/manager/interview/subject/add_subject.jsp?site_id="+site_id+"&tn="+main_type+"&topnum="+top.curTabIndex,"维护主题");
}
...
function showUpdatePate(main_type,id,audit_status)
{
	if(main_type == "main" || audit_status != 1)
	{
		...
		top.addTab(true,"/manager/interview/subject/add_subject.jsp?site_id="+site_id+"&tn="+main_type+"&id="+id+"&topnum="+top.curTabIndex,"维护主题");
	}
	else
	...
	else
	{
		...
		{			
			if(main_type == "main" || table.getSelecteBeans().get(0).audit_status != 1)
			{
				...
				top.addTab(true,"/manager/interview/subject/add_subject.jsp?site_id="+site_id+"&tn="+main_type+"&id="+selectIDS+"&topnum="+top.curTabIndex,"维护主题");
			}
			else
			{
				top.msgAlert("该主题已经通过审核，不能再修改操作");
...
{	
	...
	top.addTab(true,'/manager/interview/subject/view_subject.jsp?tn='+main_type+'&id='+id,'查看访谈主题');
}
...
{	
	...
	top.addTab(true,'/manager/interview/subject/actorList.jsp?&tn='+main_type+'&sub_id='+selectBean.get(0).sub_id+'&ss='+selectBean.get(0).audit_status,'嘉宾资料管理');	
}
...
function fnReleInfoManager(main_type)
{
	...
	else
	{
		...
		top.addTab(true,"/manager/interview/subject/releInfoList.jsp?&tn="+main_type+"&sub_id="+ls.get(0).sub_id+"&ss="+ls.get(0).audit_status,"相关信息管理");
	}
}
...
{
	top.addTab(true,"/manager/interview/subject/search_subject.jsp?tn="+main_type+"&ti="+top.curTabIndex,"访谈主题查询");
}
