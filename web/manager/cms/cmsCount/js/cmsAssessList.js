...
function showList(){
	...
	var mp = new Map();
	// 不包含引用信息
	if(!$("#ref_info").attr("checked")){
		// 不包含引用也不包含链接信息
		if(!$("#link_info").attr("checked")){
			is_host = "0";
			mp.put("is_host","0");
		}else{ // 不包含引用,包含连接
			is_host = "0,2";
...
function openPieChart(input_user,user_name){
	...
	//top.OpenModalWindow("查看详细信息---"+user_name,"/manager/cms/cmsCount/cmsAssessUserInfo.jsp?site_id="+site_id+"&input_user="+input_user,1000,600);
	top.addTab(true,"/manager/cms/cmsCount/cmsAssessUserInfo.jsp?site_id="+site_id+"&input_user="+input_user+"&start_day="+start_day+"&end_day="+end_day+"&selectedId="+selected_ids+"&is_host="+is_host,"查看详细信息---"+user_name);
}
