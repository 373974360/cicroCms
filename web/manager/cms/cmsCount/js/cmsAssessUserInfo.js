...
function showList(){
	...
	/*
	// 不包含引用信息
	if(!top.getCurrentFrameObj().$("#ref_info").attr("checked")){
		// 不包含引用也不包含链接信息
		if(!top.getCurrentFrameObj().$("#link_info").attr("checked")){
			mp.put("is_host","0");
		}else{ // 不包含引用,包含连接
			mp.put("is_host","0,2");
		}
	...
	// 判断栏目选择信息
	//var statu = top.getCurrentFrameObj().$("#all_cat_ids").attr("checked");
	//if(statu) {
...
function openPieChart(input_user) {
	...
	top.OpenModalWindow("查看详细信息","/manager/zwgk/count/chart.html",475,470);
	
}

// 处理全选事件
function allSelected(){
	var statu = $("#all_cat_ids").attr("checked");
	if(statu){
...
{
  top.OpenModalWindow("详细信息列表","/manager/cms/cmsCount/UserInfoList.jsp?site_id="+site_id+"&input_user="+input_user+"&start_day="+start_day+"&end_day="+end_day+"&is_host="+is_host+"&cat_id="+cat_id,1000,600);
}