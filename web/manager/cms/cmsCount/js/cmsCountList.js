...
function showList(){
	...
	mp.put("end_day",b_end_day + " 23:59:59");
	var v_type = $(":radio[name='viewClass'][checked]").val();
	if( v_type != "bycat"){
		mp.put(v_type,v_type);
	}
	beanList = CmsCountRPC.getInfoCount(mp);
	beanList = List.toJSList(beanList);
	
	if( v_type != "bycat"){
		createTableByDate(beanList);
	}else{
		createTable(beanList);
	}
	
	setInfoTotal(beanList);
...
function searchBtn(){
	...
	if(beanList.size() != 0) {
		...
		listHeader.add("引用信息");
		listHeader.add("连接信息"); 
		fileUrl = CmsCountRPC.cmsInfoOutExcel(beanList,listHeader);
	}
}
...
function setSearchInfo(){
	var isInput = $("#isInput").attr("checked");
	// 选中输入时间
	if(isInput){
		b_start_day = $("#start_day").val();
		b_end_day = $("#end_day").val()+"";
	}else {
		if($("#time_month").val() ==0 ){
			b_start_day = $("#time_year").val() + "-01-01";
			b_end_day = $("#time_year").val() + "-12-31";
		}else{
			b_start_day = $("#time_year").val()+"-"+$("#time_month").val()+"-01";			
			b_end_day = $("#time_year").val()+"-"+$("#time_month").val()+"-" +
				lastDayOfMonth($("#time_month").val(),$("#time_year").val());
		}
	}
	$("#s_day").html(b_start_day);
	$("#e_day").html(b_end_day);
}

// 根据输入时间段的checked选中情况,改变输入框的隐藏状态
...
// 按日,按月统计时生成统计页table
function createTableByDate(beanList)
{
	$("#countList").empty();
	var treeHtmls = "<thead>" +
    "<tr> " +
	...
	if(beanList.size() != 0) {
		for(var i=0;i<beanList.size();i++){
			treeHtmls += stuffTableByDate(beanList.get(i));
		}
		treeHtmls += "</tbody>"+
		"<tfoot>" +
 	 	"<td colspan=\"5\"></td>"+ " </tfoot>";
	} else {
		treeHtmls += "<tr><td colspan=\"5\" width=\"806\">暂无数据...</td></tr>" + "</tbody>"+"<tfoot>" +"<td colspan=\"5\"></td>"+" </tfoot>";
	}
	$("#countList").append(treeHtmls);
	iniTreeTable("countList");
...
// 填充每一行的具体数据
// 添加数据的聚合,显示信息总数
function stuffTableByDate(bean)
{			
	var treeHtml = "";
	var type_calss= "";
	treeHtml+="<tr> <td><span class='file'>"+bean.time+"</span></td>";
    treeHtml += "<td>"+ bean.allCount +"</td>";
    treeHtml += "<td>"+ bean.hostInfoCount +"</td>";
    treeHtml += "<td>"+ bean.refInfoCount +"</td>";
    treeHtml += "<td>"+ bean.linkInfoCount +"</td>";
...
function initViewType()
{
	...
	$("#bycat").click(showList);
	$("#byday").click(showList);
	$("#bymonth").click(showList);
}
...
function setBarChart()
{
	...
	if(beanList.size() == 0)
	{
		...
		//swfobject.embedSWF("/manager/js/open-flash-chart/open-flash-chart.swf", "chart", wid, "400", "9.0.0"); 
		return "";
	}
	...
	chartJsonData = '{   '+     
		...
		']}';
	swfobject.embedSWF("/manager/js/open-flash-chart/open-flash-chart.swf", "chart", wid, "400", "9.0.0",null, null,{wmode:'transparent'}); 
}
