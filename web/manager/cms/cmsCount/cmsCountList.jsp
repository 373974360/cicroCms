...
function setCountType(){
    if($("#count_type").val()=="1"){
          window.location.href="/manager/cms/cmsCount/cmsCountListCate.jsp?site_id="+site_id;
	}
}
</script>
</head>
...
<table style="width:100%" border="0" cellpadding="0" cellspacing="0" >
	...
	<tr>
    	<td colspan="3"  class="search_td">
			...
			%>
            	<option value="<%=cur_year%>" selected="selected"><%=cur_year%></option>
            <%
				for(int j=1; j<=5; j++){
			%>
            	<option value="<%=cur_year-j%>"><%=cur_year-j%></option>
            <%
                }
			%>
            </select>年
            ...
            <input class="Wdate" type="text" name="start_day" id="start_day" size="11" style="height:16px;line-height:16px;" 
					value="<%=start_day%>" onFocus="WdatePicker({isShowClear:true,readOnly:true,dateFmt:'yyyy-MM-dd'})" readonly="true" disabled="disabled" onchange="setSearchInfo()"/>
		         --
			    <input class="Wdate" type="text" name="end_day" id="end_day" size="11" style="height:16px;line-height:16px;" 
					 value="<%=end_day%>" onFocus="WdatePicker({isShowClear:true,readOnly:true,dateFmt:'yyyy-MM-dd'})" readonly="true" disabled="disabled" onchange="setSearchInfo()"/>
	...
	<tr>
     ...
     <td>
       ...
       <table width="100%"> 
			<tbody>
				<tr> <td width="60px">查看方式:</td><td>
				<input id="bycat" type="radio" name="viewClass" checked="checked" value="bycat">按栏目 
				<input id="byday" type="radio" name="viewClass" value="byday" disabled="disabled" >按日
				<input id="bymonth" type="radio" name="viewClass" value="bymonth">按月</td></tr>

				<tr><td>时间范围:</td><td><span id='s_day'></span>至<span id='e_day'></span></td></tr>

				<tr><td>统计情况:</td><td>全部信息:(<span id="allInfo">0</span>)&nbsp;主信息:(<span id="hostInfo">0</span>)&nbsp;引用信息:(<span id="refInfo">0</span>)&nbsp;链接信息:(<span id="linkInfo">0</span>)</td></tr>
</tbody>
</table>
       </div>
       </td>
