...
function showList(){	
	...
	table.getCol("cat_handl").each(function(i){
		if(i>0)
		{
			$(this).html('<a href="javascript:top.addTab(true,\'/manager/cms/category/categoryList.jsp?cat_type=2&site_id='+site_id+'&app_id='+app_id+'&cat_id='+beanList.get(i-1).cat_id+'\',\'目录管理\')">目录管理</a>');	
		}
	});
...
{
	top.addTab(true,"/manager/cms/category/category_add.jsp?site_id="+site_id+"&app_id="+app_id+"&top_index="+top.curTabIndex+"&&parentID=0&cat_type=2","维护目录");
}
...
function openUpdateCategoryPage(cid)
{
	...
	top.addTab(true,"/manager/cms/category/category_add.jsp?site_id="+site_id+"&app_id="+app_id+"&top_index="+top.curTabIndex+"&id="+id+"&cat_type=2","维护目录");
}
