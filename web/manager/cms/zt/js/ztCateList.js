...
function showList()
{
	...
	table.getCol("zt_cat_name").each(function(i){
		if(i>0)
		{
			$(this).html('<a href="javascript:top.OpenModalWindow(\'分类方法信息\',\'/manager/cms/zt/ztCate_add.jsp?type=update&id='+beanList.get(i-1).id +'\',440,130)">'+beanList.get(i-1).zt_cat_name+'</a>');	
		}
	});
...
function addZTCate()
{
	top.OpenModalWindow("新建分类方式","/manager/cms/zt/ztCate_add.jsp?type=add&id=",440,130);
}
...
function updateZTCate()
{
	...
	top.OpenModalWindow("修改分类方式","/manager/cms/zt/ztCate_add.jsp?type=update&id="+id,440,130);
}
