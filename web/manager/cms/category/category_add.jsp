...
var cat_id;
var CategoryGetRegu = new Bean("com.cicro.wcm.bean.cms.category.CategoryGetRegu",true);
var defaultBean;
...
$(document).ready(function(){
	...
	if(class_id == 0)//为0表示是从站点中添加的，要打开共享目录的窗口，否则打开选择基础目录的窗口
		$("#selectCateClassButton").click(function(){openSelectSingleShareCateClass('选择类目','releCateClass',app_id)});
	else
...
{
	var is_auto_publish = $("#is_auto_publish").attr("checked") == true ? 1:0;	
...
{	
	if($("#"+table_name+" :radio[checked]").val() == 1)
	{//1为全部共享 
		csb.range_type = 1;
		if(is_update)
...
{
	$(obj).prev().attr("checked",$(obj).prev().attr("checked") == false ? true:false);
	$(obj).prev().click();
}
...
<table id="category_table" class="table_form" border="0" cellpadding="0" cellspacing="0">
	<tbody>
		...
		<tr>
			...
			<td colspan="3">
				<textarea id="cat_keywords" name="cat_keywords" style="width:585px;height:50px;" onblur="checkInputValue('cat_keywords',true,8000,'关键词','')"></textarea>		
			</td>
		</tr>
		<tr>
			<th style="vertical-align:top;">描述：</th>
			<td colspan="3">
				<textarea id="cat_description" name="cat_description" style="width:585px;height:50px;" onblur="checkInputValue('cat_description',true,8000,'描述','')"></textarea>		
			</td>
		</tr>
		<tr>
			<th style="vertical-align:top;">备注：</th>
			<td colspan="3">
				<textarea id="cat_memo" name="cat_memo" style="width:585px;height:50px;" onblur="checkInputValue('cat_memo',true,8000,'备注','')"></textarea>		
			</td>
		</tr>
...
<table id="share_table" class="table_form" border="0" cellpadding="0" cellspacing="0">
	<tbody>
		<tr>
			<th>共享范围：</th>
			<td>
				<input id="is_shared" name="is_shared" type="radio" value="1" onclick="disabledSiteChekced(true,'share_table')"/><label>全部共享</label>
				<input id="is_shared" name="is_shared" type="radio" value="0" checked="true" onclick="disabledSiteChekced(false,'share_table')"/><label>限制共享</label>
			</td>			
		</tr>
		<tr>
		   <th></th>
...
<table id="receive_table" class="table_form" border="0" cellpadding="0" cellspacing="0">
	<tbody>
		<tr>
			<th>接收范围：</th>
			<td>
				<input id="is_shared_1" name="is_shared_1" type="radio" value="1" onclick="disabledSiteChekced(true,'receive_table')"/><label>全部接收</label>
				<input id="is_shared_1" name="is_shared_1" type="radio" value="0" checked="true" onclick="disabledSiteChekced(false,'receive_table')"/><label>限制接收</label>
			</td>			
		</tr>
		<tr>
		   <th></th>
...
<table id="sync_table" class="table_form" border="0" cellpadding="0" cellspacing="0">
	<tbody>
		<tr>
			<th>选择站点：</th>
			<td>
				<select id="shared_site_list" class="width300" onchange="getSharedCategoryBySite(this.value)"><option value="">当前站点</option></select>
			</td>			
		</tr>
		<tr>
		   <th></th>
...
<table class="" border="0" cellpadding="2" cellspacing="0" align="left">
	<tr>
	  ...
	  <td valign="top">	
			<div id="scroll_div" style="width:397px;height:194px;overflow:auto;background:#ffffff" class="border_color">
				<ul id="data_node" class="txt_list" style="width:1000px;">

				</ul>
			</div><br/>
			<span style="color:#32609E">共享目录</span>
			<div id="scroll_div" style="width:397px;height:180px;overflow:auto;background:#ffffff;padding-top:6px" class="border_color">
...
<table id="sync_table" class="table_form" border="0" cellpadding="0" cellspacing="0">
	<tbody>
		<tr>
			<th>选择站点：</th>
			<td>
				<select id="toInfo_site_list" class="width300" onchange="getToInfoCategoryBySite(this.value)"></select>
			</td>			
		</tr>
		<tr>
		   <th></th>
