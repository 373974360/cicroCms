...
var CateClassBean = new Bean("com.cicro.wcm.bean.cms.category.CateClassBean",true);
...
function showList()
{
	...
	table.getCol("class_cname").each(function(i){
		if(i>0)
		{
			$(this).html('<a href="javascript:top.OpenModalWindow(\'分类方法信息\',\'/manager/cms/cateClass/cateClass_add.jsp?type=update&class_id='+beanList.get(i-1).class_id +'\',460,385)">'+beanList.get(i-1).class_cname+'</a>');	
		}
	});
	...
	table.getCol("cat_handl").each(function(i){
		if(i>0)
		{
			$(this).html('<a href="javascript:top.addTab(true,\'/manager/cms/category/categoryShareList.jsp?class_id='+beanList.get(i-1).class_id+'\',\'目录管理\')">目录管理</a>');	
		}
	});
...
{
	top.OpenModalWindow("新建分类","/manager/cms/cateClass/cateClass_add.jsp?type=add&class_id=",460,385);
}
...
function updateCateClass()
{
	...
	top.OpenModalWindow("修改分类","/manager/cms/cateClass/cateClass_add.jsp?type=update&class_id="+class_id,460,385);
}
