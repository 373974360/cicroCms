...
{	
	...
	for(var i=0;i<beanList_cus.size();i++){
		var field_enname = beanList_cus.get(i).field_enname.toLocaleLowerCase();
	...
	//添加主信息表
	publicSaveInfoEventCustom(bean,cusBean,"article_custom","insert");	
}
...
function updateInfoDataCustom()
{
	...
	publicSaveInfoEventCustom(bean,cusBean,"article_custom","update");	
...
function publicSaveInfoEventCustom(bean,cusBean,model_ename,save_type)
{
	...
	if(save_type == "update")
	{
		...
		{//修改引用此信息的信息
			if(model_ename != "link") 
				jsonrpc.InfoCustomRPC.updateQuoteInfoCustom(bean,cusBean,model_ename);
		}
	}
	else
	{
