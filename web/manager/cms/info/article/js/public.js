...
var imgDomain = "";
var KnowledgeRPC = jsonrpc.KnowledgeRPC; // add by zhangqiang 20130907
...
function addRelatedHand(){
	...
	top.OpenModalWindow("手动添加","/manager/cms/info/article/addRelatedInfo.jsp?info_id="+infoid+"&site_id="+site_id,400,200);	
}
...
{	
	...
	if(t_site_id != "" && t_site_id != null)
	{
		...
		temp_site = site_id;
	top.OpenModalWindow("选择相关信息","/manager/cms/info/article/chooseInfoPage.jsp?info_id="+Info_id+"&site_id="+temp_site,825,500);	
}
...
{	
	...
	if(app_id == "zwgk")
	{
		...
		temp_site = site_id;
	top.OpenModalWindow("选择手动标签","/manager/cms/info/article/focusInfo.jsp?info_id="+Info_id+"&site_id="+temp_site+"&t="+tmp,660,500);	
}
...
{
	//window.location.href = "/manager/cms/info/article/articleDataList.jsp?cat_id="+bean.cat_id+"&app="+bean.app_id+"&site_id="+bean.site_id+"&snum="+snum;
...
//	bean.title = bean.title.replace(/\"/g,"＂");
	...
	if(save_type == "update")
	{
		...
		insertInfoRelatedKcat(bean);//2013-09-09 添加知识库信息
...
//知识库标签入库 -- start 
function insertInfoRelatedKcat(bean)
{
	var kcat_map = new Map();

	var kcat_name = $("#kcat_name").val()+"";
	var kcat_id = $("#kcat_id").val()+"";
	$("#kcat_name").val(kcat_name);
	$("#kcat_id").val(kcat_id);

	if(kcat_name !="" && kcat_id !="")
	{
		kcat_map.put("kcat_id",kcat_id);
		kcat_map.put("info_id",bean.info_id+"");
		kcat_map.put("site_id",site_id);
		kcat_map.put("app_id",app_id);

		InfoRelatedKcatRPC.insertInfoRelatedKcat(kcat_map);
	}
}
//知识库标签入库 --end 
...
function setUserOperate()
{
	...
	{	//是否有发布权限
		...
		if(wf_id != 0 && opt_ids.indexOf(",303,") > -1)
		{
