...
//var RelatedBean = new Bean("com.cicro.wcm.bean.cms.info.RelatedInfoBean",true);
...
function initTable(){
	...
	colsList.add(setTitleClos("modify_user","修改人","60px","","",""));
...
//	m.put("is_host", app);
	...
	var um = UserManRPC.getSimpleUserMap();
	um = Map.toJSMap(um);
	...
	table.getCol("input_user").each(function(i){		
		...
		{	
			...
			{
				if(um.get(beanList.get(i-1).input_user+"") != null && um.get(beanList.get(i-1).input_user+"") != "")
				{
					$(this).text(um.get(beanList.get(i-1).input_user+"").user_realname);	
				}
				else
				{
					$(this).text("");	
	...
	table.getCol("modify_user").each(function(i){		
		...
		{	
			...
			{
				if(um.get(beanList.get(i-1).modify_user+"") != null && um.get(beanList.get(i-1).modify_user+"") != "")
				{
					$(this).text(um.get(beanList.get(i-1).modify_user+"").user_realname);	
				}
				else
				{
					$(this).text("");	
	...
	table.getCol("opt_time").each(function(i){
		...
		{			
			...
			switch(nm){
				...
				case 1:str = "送审时间";break;
				case 2:str = "送审时间";break;
				case 3:str = "撤销时间";break;
				case 4:str = "审核时间";break;
				case 8:str = "发布时间";break;
				case 9:str = "删除时间";break;
	...
	table.getCol("actions").each(function(i){
		...
		{	
			...
			case 1:
				...
				str += "<li id='332' class='ico_delete' ><a title='删除' href='javascript:doDelete("+(i-1)+")' style='width:16px;height:16px;'>&nbsp;&nbsp;&nbsp;&nbsp;</a></li> ";break;
			case 2:
				str += "<li id='303' class='ico_pass'><a title='通过' href='javascript:doPass("+(i-1)+")' style='width:16px;height:16px;'>&nbsp;&nbsp;&nbsp;&nbsp;</a></li><li id='303' class='ico_nopass'><a  title='退稿' href='javascript:noPassDesc("+beanList.get(i-1).info_id+")' style='width:16px;height:16px;'>&nbsp;&nbsp;&nbsp;&nbsp;</a></li><li id='315' class='ico_edit' ><a  title='修改' href='javascript:openUpdatePage("+beanList.get(i-1).info_id+","+beanList.get(i-1).model_id+","+beanList.get(i-1).is_host+")' style='width:16px;height:16px;'>&nbsp;&nbsp;&nbsp;&nbsp;</a></li> <li id='301' class='ico_delete' ><a title='删除' href='javascript:doDelete("+(i-1)+")' style='width:16px;height:16px;'>&nbsp;&nbsp;&nbsp;&nbsp;</a></li> ";break;
			case 3:
...
function clickLabelHandl(num)
{
	...
	case 1:
		...
		finalStatus = "0";
		search_steps = "";
		table.sortCol="ci.opt_dtime desc,ci.id";
		table.sortType = "desc";
		break;
	case 2:
		infoStatus = "3";
...
{		
	...
	if(app == "ggfw")
	{
		top.addTab(true,"/manager/cms/info/article/m_link.jsp?cid="+cid+"&site_id="+site_id+"&app_id="+app+"&model=12&topnum="+top.curTabIndex,"添加信息");
	}
	else
	{
		if(gk_article == true)
		{
			top.addTab(true,"/manager/cms/info/article/m_gk_gkzn.jsp?cid="+cid+"&site_id="+site_id+"&app_id="+app+"&model="+model_id+"&topnum="+top.curTabIndex,"添加信息");
		}
		else
			top.addTab(true,"/manager/cms/info/article/"+getAddPagebyModel(model_id)+"?cid="+cid+"&site_id="+site_id+"&app_id="+app+"&model="+model_id+"&topnum="+top.curTabIndex,"添加信息");
	}
}
...
function openUpdatePage(Infoid,model_id,is_host)
{
	...
	{
		//引用信息只修改信息主表内容
		top.addTab(true,"/manager/cms/info/article/update_info.jsp?cid="+cid+"&info_id="+Infoid+"&site_id="+site_id+"&app_id="+app+"&model="+model_id+"&topnum="+top.curTabIndex,"维护信息");
	}
	else
	{
		if(gk_article == true)
		{
			top.addTab(true,"/manager/cms/info/article/m_gk_gkzn.jsp?cid="+cid+"&info_id="+Infoid+"&site_id="+site_id+"&app_id="+app+"&model="+model_id+"&topnum="+top.curTabIndex,"维护信息");
		}else
			top.addTab(true,"/manager/cms/info/article/"+getAddPagebyModel(model_id)+"?cid="+cid+"&info_id="+Infoid+"&site_id="+site_id+"&app_id="+app+"&model="+model_id+"&topnum="+top.curTabIndex,"维护信息");
	}
}

function openViewPage(i_id)
{
	//window.location.href = "/manager/cms/info/article/infoView.jsp?info_id="+i_id+"&site_id="+site_id+"&snum="+snum;
...
function openWindowForPush(){		
	top.OpenModalWindow("信息推送","/manager/cms/info/article/infoTS.jsp?site_id="+site_id+"&app_id="+app,400,480);
}

//转移
function openWindowForMov(){	
	top.OpenModalWindow("信息转移","/manager/cms/info/article/infoMove.jsp?site_id="+site_id+"&app_id="+app,400,460);	
}
...
function noPassDesc(id)
{
	if(id != null && id != "")
		temp_info_id = id;
	top.OpenModalWindow("退稿意见","/manager/cms/info/article/noPassDesc.jsp",520,235);	
}
...
function getInfoFromOtherCat(){
	//打开页面
	is_save_first_page = true;
	top.OpenModalWindow("信息获取","/manager/cms/info/article/infoGet.jsp?cat_id="+cid+"&site_id="+site_id+"&app_id="+app,800,530);	
}
...
{
	if(app == "cms")
		top.OpenModalWindow("高级搜索","/manager/cms/info/article/infoSearch.jsp",340,350);	
	if(app == "zwgk")
		top.OpenModalWindow("高级搜索","/manager/cms/info/article/gkInfoSearch.jsp",590,510);	
}
...
{
	is_save_first_page = true;
	top.OpenModalWindow("信息获取","/manager/cms/info/article/getAllocateInfo.jsp?cat_id="+cid+"&site_id="+site_id+"&app_id="+app,800,530);	
}
