...
$(document).ready(function(){	
	...
	if(defaultBean != null)
	{
		...
		}else
		{
			...
			{
				if(defaultBean.info_status == 8)//如果信息已经发布
				{
					var wf_id = CategoryRPC.getWFIDByCatID(defaultBean.cat_id,site_id);//判断栏目是否有审核
					if(wf_id == 0)
						$("#btn300").show();
					else
					{
						var step_id = getMaxStepIDByUserID(wf_id,app_id,site_id);
						if(step_id == 100)
							$("#btn300").show();
					}
				}
...
function openUpdatePage()
{
	...
	if(defaultBean.is_host == 1)
	{		
		window.location.href = "/manager/cms/info/article/update_info.jsp?cid="+defaultBean.cat_id+"&info_id="+info_id+"&site_id="+defaultBean.site_id+"&app_id="+defaultBean.app_id+"&model="+defaultBean.model_id+"&topnum="+topnum;
	}else
	{
		switch(defaultBean.cat_id)
		{
			case 10:window.location.href = "/manager/cms/info/article/m_gk_gkzn.jsp?cid="+defaultBean.cat_id+"&site_id="+defaultBean.site_id+"&app_id="+defaultBean.app_id+"&topnum="+topnum;
					break;
			case 11:window.location.href = "/manager/cms/info/article/m_gk_gkzn.jsp?cid="+defaultBean.cat_id+"&site_id="+defaultBean.site_id+"&app_id="+defaultBean.app_id+"&topnum="+topnum;
					break;
			case 12:window.location.href = "/manager/cms/info/article/m_gk_gkzn.jsp?cid="+defaultBean.cat_id+"&info_id="+defaultBean.info_id+"&site_id="+defaultBean.site_id+"&app_id="+defaultBean.app_id+"&model="+defaultBean.model_id+"&topnum="+topnum;
					break;
			default:window.location.href = "/manager/cms/info/article/"+getAddPagebyModel(defaultBean.model_id)+"?cid="+defaultBean.cat_id+"&info_id="+info_id+"&site_id="+site_id+"&app_id="+defaultBean.app_id+"&topnum="+topnum;
		}	
	}
}
...
function openFromInfoPage()
{
	...
	{
		window.location.href = "/manager/cms/info/article/"+getAddPagebyModel(from_info_bean.model_id)+"?cid="+from_info_bean.cat_id+"&info_id="+defaultBean.from_id+"&site_id="+from_info_bean.site_id+"&app_id="+from_info_bean.app_id+"&topnum="+topnum;
	}
}
