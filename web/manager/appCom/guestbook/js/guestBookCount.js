...
function showList(){			
	...
	beanList = GuestBookRPC.getGuestbookStatistics(m);//第一个参数为站点ID，暂时默认为空	
...
function getAllSubList()
{
	...
	beanList = GuestBookRPC.getAllGuestBookSubList(site_id);
	beanList = List.toJSList(beanList);

	for (var i = 0; i < beanList.size(); i++) {
		tableTr += '<li><input style="vertical-align:middle" type="checkbox" id="gbs_id" name="b_id" onclick="setAllState(this)" value="'+beanList.get(i).gbs_id+'"><label>'+beanList.get(i).title+'</label></li>';
	}
	$("#b_tr").append(tableTr);
}
