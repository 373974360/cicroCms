...
var RuleCategoryBean = new Bean("com.cicro.wcm.dataCollection.bean.RuleCategoryBean", true);
...
function showList()
{
	...
	table.getCol("rcat_name").each(function(i){
		if(i>0)
		{
			$(this).html('<a href="javascript:top.OpenModalWindow(\'新建采集规则分类\',\'/manager/dataCollection/ruleCategoryAdd.jsp?type=update&id='+beanList.get(i-1).id+'\',450,230)">'+beanList.get(i-1).rcat_name+'</a>');
		}
	});
...
{
	top.OpenModalWindow("新建采集规则分类","/manager/dataCollection/ruleCategoryAdd.jsp?type=add&id="+id,450,230);
}
...
function openUpdatePage()
{
	...
	top.OpenModalWindow("新建采集规则分类","/manager/dataCollection/ruleCategoryAdd.jsp?type=update&id="+selectID,450,230);
}
...
function saveUpdateRuleCategory()
{
	...
	if(CollectionDataRPC.updateRuleCategory(updateBean))
	{
