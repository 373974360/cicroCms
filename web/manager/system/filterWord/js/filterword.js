...
var FilterWordBean = new Bean("com.cicro.wcm.bean.system.filterWord.FilterWordBean",true);
...
function showList(){
	...
	table.getCol("pattern").each(function(i){
		...
		{			
			...
			$(this).html('<a href="javascript:top.OpenModalWindow(\'维护过滤词\',\'/manager/system/filterWord/filterWord_add.jsp?fw_id=' + beanList.get(i-1).filterword_id + '\',450,190);">'+beanList.get(i-1).pattern+'</a>');
		}
	});	
...
{	
	top.OpenModalWindow("维护过滤词","/manager/system/filterWord/filterWord_add.jsp?fw_id="+fw_id,450,190);	
}

//打开添加窗口
function openAddFilterWordPage(app_id,site_id)
{
	top.OpenModalWindow("维护过滤词","/manager/system/filterWord/filterWord_add.jsp?app_id="+app_id+"&site_id="+site_id,450,190);	
}
...
function openUpdateFilterWordPage()
{
	...
	top.OpenModalWindow("维护过滤词","/manager/system/filterWord/filterWord_add.jsp?fw_id="+selectIDS,450,190);
}
