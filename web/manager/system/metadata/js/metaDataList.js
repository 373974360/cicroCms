...
var MetaDataBean = new Bean("com.cicro.wcm.bean.system.metadata.MetaDataBean",true);
...
function showList(){	
	...
	table.getCol("meta_cname").each(function(i){
		...
		{			
			...
			$(this).html('<a href="javascript:top.addTab(true,\'/manager/system/metadata/metaData_view.jsp?meta_id='+beanList.get(i-1).meta_id+'\',\'元数据信息\')">'+beanList.get(i-1).meta_cname+'</a>');
		}
	});	
...
{	
	window.location.href = "/manager/system/metadata/metaData_view.jsp?meta_id="+r_id;
}

//打开添加窗口
function openAddMetaDataPage()
{
	window.location.href = "/manager/system/metadata/metaData_add.jsp";
}
...
function openUpdateMetaDataPage()
{
	...
	window.location.href = "/manager/system/metadata/metaData_add.jsp?meta_id="+selectIDS;
}
