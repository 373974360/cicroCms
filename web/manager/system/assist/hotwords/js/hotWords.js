...
var HotWordBean = new Bean("com.cicro.wcm.bean.system.assist.HotWordBean",true);
...
function showList(){	
	...
	table.getCol("hot_url").each(function(i){
		...
		{			
			...
			//$(this).html('<a href="javascript:top.addTab(true,\'/manager/system/assist/hotwords/hotWord_view.jsp?hot_id='+beanList.get(i-1).hot_id+'\',\'元数据信息\')">'+beanList.get(i-1).hot_name+'</a>');
		}
	});	
...
{	//OpenModalWindow
	top.OpenModalWindow("热词查看","/manager/system/assist/hotwords/hotWord_view.jsp?hot_id="+hot_id,360,165);	
}

//打开添加窗口
function openAddHotPage()
{
	top.OpenModalWindow("维护热词","/manager/system/assist/hotwords/hotWord_add.jsp?app="+app+"&site_id="+site_id,360,165);	
}
...
function openUpdateHotDataPage()
{
	...
	top.OpenModalWindow("维护热词","/manager/system/assist/hotwords/hotWord_add.jsp?hot_id="+selectIDS,360,165);
}

function openUpdatePage(hotid){
	top.OpenModalWindow("维护热词","/manager/system/assist/hotwords/hotWord_add.jsp?hot_id="+hotid,360,165);
}
