...
var WareCategoryBean = new Bean("com.cicro.wcm.bean.system.ware.WareCategoryBean", true);
...
function showList()
{
	...
	table.getCol("wcat_name").each(function(i){	
		if(i>0)
		{
			$(this).html('<a href="javascript:top.OpenModalWindow(\'新建标签分类\',\'/manager/system/ware/wareCategoryAdd.jsp?type=update&id='+beanList.get(i-1).id+'\',450,230)">'+beanList.get(i-1).wcat_name+'</a>');
		}
	});
...
{
	top.OpenModalWindow("新建标签分类","/manager/system/ware/wareCategoryAdd.jsp?type=add&id="+id,450,230);
}
...
function openUpdatePage()
{
	...
	top.OpenModalWindow("新建标签分类","/manager/system/ware/wareCategoryAdd.jsp?type=update&id="+selectID,450,230);
}
