...
var ModelBean = new Bean("com.cicro.wcm.bean.system.formodel.ModelBean",true);
...
function showList(){		
	...
	table.getCol("model_name").each(function(i){
		if(i>0)
		{			
			$(this).html('<a target="_self" href="/manager/system/formodel/model_add.jsp?model_id='+beanList.get(i-1).model_id+'">'+beanList.get(i-1).model_name+'</a>');			
		}
	});*/	
	...
	table.getCol("action_Col").each(function(i){
		if(i>0)
		{			
			$(this).html('<a href="javascript:top.addTab(true,\'/manager/system/formodel/fieldList.jsp?model_id='+beanList.get(i-1).model_id+'\',\'内容模型字段\')">字段维护</a>');
		}
	});	
	...
	table.getCol("action").each(function(i){
		...
		{	
			...
			if(beanList.get(i-1).model_type == "0"){
				...
				$(this).html('<a href="javascript:top.addTab(true,\'/manager/model/form/form_list.jsp?model_id='+beanList.get(i-1).model_id+'\',\'内容模型字段\')">字段维护</a>');
			}else if(beanList.get(i-1).model_type == "2"){
				//$(this).html("第三方");
				$(this).html('<a href="javascript:top.addTab(true,\'/manager/model/form/form_list3.jsp?model_id='+beanList.get(i-1).model_id+'\',\'内容模型字段\')">资源库字段设置</a>');
			}
		}
	});
...
{	
	window.location.href = "/manager/system/formodel/model_view.jsp?model_id="+r_id;
}

//打开添加窗口
function openAddModelPage()
{
	window.location.href = "/manager/system/formodel/model_add.jsp";
}
...
function openUpdateModelPage()
{
	...
	window.location.href = "/manager/system/formodel/model_add.jsp?model_id="+selectIDS;
}
