...
var TemplateEditBean = new Bean("com.cicro.wcm.bean.system.template.TemplateEditBean",true);
var WareRPC = jsonrpc.WareRPC;
var WareBean = new Bean("com.cicro.wcm.bean.system.ware.WareBean", true);
var SnippetRPC = jsonrpc.SnippetRPC;
var SnippetBean = new Bean("com.cicro.wcm.bean.template.snippet.SnippetBean",true);
...
function showList(){	
	...
	table.getCol("t_cname").each(function(i){
		...
		if(i>0)
		{			
			$(this).html('<a href="javascript:window.location.href = \'/manager/system/template/template_add.jsp?t_id='+beanList.get(i-1).t_id+'&app='+beanList.get(i-1).app_id+'&site_id='+beanList.get(i-1).site_id+'&tc_id='+beanList.get(i-1).tc_id+'\'">'+beanList.get(i-1).t_cname+'</a>');
		}
	});
...
{ 
	var height = 210;
	var url ="/manager/system/template/upload.jsp?t_id="+t_id+"&site_id="+site_id+"&modify_dtime="+modify_dtime+"&t_ename="+t_ename+"&creat_dtime="+creat_dtime+"&id="+id+"&modify_user="+modify_user+"&tcat_id="+tcat_id+"&t_path="+t_path+"&t_cname="+t_cname+"&app_id="+app_id+"&t_ver="+t_ver+"&creat_user="+creat_user;
...
function openHistoryTemplateVer(templateId,siteId){
	...
	top.OpenModalWindow("模板历史版本","/manager/system/template/templateDataVerList.jsp?tc_id="+tc_id+"&t_id="+templateId+"&site_id="+siteId,1000,530);
	//window.location.href = "/manager/system/template/templateDataVerList.jsp?t_id="+templateId+"&site_id="+siteId;
}
...
{	
	window.location.href = "/manager/system/template/template_view.jsp?t_id="+t_id+"&tc_id="+tc_id;
}
...
function openAddTemplatePage()
{
	...
	window.location.href = "/manager/system/template/template_add.jsp?app="+app+"&site_id="+site_id+"&tc_id="+tc_id;
}
...
function openUpdateTemplateDataPage()
{
	...
	window.location.href = "/manager/system/template/template_add.jsp?t_id="+selectIDS+"&app="+app+"&site_id="+site_id+"&tc_id="+tc_id;
}

//添加Template
function addTemplateData()
{
	$("#t_content").val(editAreaLoader.getCode());
...
function updateTemplateData()
{
	...
	{ 
		...
		//top.CloseModalWindow();
		//top.getCurrentFrameObj().reloadTemplateDataList();
		window.location.href = "/manager/system/template/templateCategoryList.jsp?tid="+bean.tcat_id+"&site_id="+bean.site_id;
	}
	else
	{ 
		top.msgWargin("模板"+WCMLang.Add_fail);
...
function deleteTemplateDataHandl()
{
	...
	if(TemplateRPC.delTemplateEditById(selectIDS,site_id))
	{
		...
		//top.CloseModalWindow();
		top.getCurrentFrameObj().reloadTemplateDataList();
		//window.location.href = "/manager/system/template/templateCategoryList.jsp?tid="+tc_id+"&site_id="+bean.site_id;
	}else
	{
...
function deleteOneTemplateInfo(id,site_id)
{
	if(TemplateRPC.delTemplateEditById(id,site_id))
	{
		...
		//top.CloseModalWindow();
		top.getCurrentFrameObj().reloadTemplateDataList();
		//window.location.href = "/manager/system/template/templateCategoryList.jsp?tid="+tc_id+"&site_id="+site_id;
	}else
	{
		top.msgWargin("模板"+WCMLang.Delete_fail);
	}
...
function downOneTemplate(tid, siteid, appid,tpath){
	window.location.href = "/manager/system/template/downInfoFile.jsp?tid="+tid+"&site_id="+siteid+"&appid="+appid+"&tpath="+tpath;
}
