...
var TemplateCategoryBean = new Bean("com.cicro.wcm.bean.system.template.TemplateCategoryBean",true);
	...
	tf_table.getCol("file_name").each(function(i){	
		...
		{	
			if(tf_beanList.get(i-1).file_name.indexOf(".js") > -1 || tf_beanList.get(i-1).file_name.indexOf(".css") > -1)
			{
				...
				$(this).html('<a href="javascript:top.addTab(true,\'/manager/system/template/resouFile_update.jsp?tid='+tc_id+'&site_id='+site_id+'&path='+p+'\',\'维护资源\')">'+tf_beanList.get(i-1).file_name+'</a>');
			}
			else
			{
				var p = tf_beanList.get(i-1).file_path;
...
{
	top.OpenModalWindow("新建目录","/manager/system/template/add_folder.jsp",350,120);
}
...
function showTemplateCategoryList(){	
	...
	tc_table.getCol("tcat_cname").each(function(i){
		...
		if(i>0)
		{			
			$(this).html('<a href="javascript:window.location.href = \'/manager/system/template/templateCategory_add.jsp?tcat_id='+tc_beanList.get(i-1).tcat_id+'&app='+tc_beanList.get(i-1).app_id+'&site_id='+tc_beanList.get(i-1).site_id+'&pid='+tc_beanList.get(i-1).parent_id+'\'">'+tc_beanList.get(i-1).tcat_cname+'</a>');
		}
	});
...
{
	top.insertTreeNode(eval('[{"id":'+id+',"iconCls":"tree-file icon-templateFolder", "text":\"'+name+'\","attributes":{"url":\"/manager/system/template/templateCategoryList.jsp?tid='+id+'\"}}]'));	
}
//修改树节点
...
{	
	window.location.href = "/manager/system/template/templateCategory_view.jsp?pid="+parent_id;
}

//打开添加窗口
function openAddTemplateCategoryPage()
{
	window.location.href = "/manager/system/template/templateCategory_add.jsp?app="+app+"&site_id="+site_id+"&pid="+parent_id;
}
...
function openUpdateTemplateCategoryDataPage()
{
	...
	window.location.href = "/manager/system/template/templateCategory_add.jsp?tcat_id="+selectIDS+"&app="+app+"&site_id="+site_id+"&pid="+parent_id;
}
...
function addTemplateCategoryData()
{
	...
	if(TemplateRPC.addTemplateCategory(bean))
	{
		...
		window.location.href = "/manager/system/template/templateCategoryList.jsp?tid="+bean.parent_id+"&labNum=2&site_id="+bean.site_id;
	}
	else
	{
...
function deleteTemplateCategoryData()
{
	...
	if(TemplateRPC.delTemplateCategoryById(selectIDS,site_id))
	{
		...
		//window.location.href = "/manager/system/template/templateCategoryList.jsp?tid="+parent_id+"&labNum=2&site_id="+bean.site_id;
	}else
	{
