...
function initEditArea(textareaID)
{
	...
	*/
	editAreaLoader = CodeMirror.fromTextArea(textareaID, {
		height: "413px",
		width: "97%",
		//parserfile: "/manager/js/mCodeArea/js/parsexml.js",
		//stylesheet: "/manager/js/mCodeArea/css/xmlcolors.css",
		parserfile: ["/manager/js/mCodeArea/js/parsexml.js", "/manager/js/mCodeArea/js/parsecss.js", "/manager/js/mCodeArea/js/tokenizejavascript.js", "/manager/js/mCodeArea/js/parsejavascript.js", "/manager/js/mCodeArea/js/parsehtmlmixed.js"],
		stylesheet: ["/manager/js/mCodeArea/css/xmlcolors.css", "/manager/js/mCodeArea/css/jscolors.css", "/manager/js/mCodeArea/css/csscolors.css"],
		path: "js/",
...
//于多超2012年1月8号修改
function insertWareTag_F(id,ware_name,wcat_position){
	  var c_name_F = wcat_position+">>";   
	  if(wcat_position.substr(0,1)=="$")
	  {
	   c_name_F=""; 
	   var strs= new Array();
	   strs=wcat_position.split("\$"); //层级 既：分类表ID
	   var wcmbean_F;
		
	   for (i=1;i<strs.length-1;i++)
	   {
		  if(strs[i]!="") { 
		  wcmbean_F = WareRPC.getWareCategoryByID(strs[i]);
		  c_name_F += wcmbean_F.wcat_name+">>";//分类名称
		  } 
	   }
	  }
	var text = "<!-- ---- "+c_name_F+"--【"+ware_name+"】 -- 标签开始 ---- -->\n<!"+"--"+"#"+"include virtual='/include/ware_"+id+".htm'-->\n<!-- ---- "+c_name_F+"--【"+ware_name+"】 -- 标签结束 ---- -->\n";//wareMap.get(id).ware_content;
	//editAreaLoader.insertTags(areaId, text, "");
	editAreaLoader.replaceSelection(text);
}
...
function showWareBeanList(wid){
	...
	//2014-10-09号注释，因为编辑一个模板请求超过400次，严重bug
	/*
	for(var i=0; i<wBeanList.size(); i++){
		wareMap.put(wBeanList.get(i).ware_id,wBeanList.get(i));
		//*********2013-1-9添加
		 var wcat_id = wBeanList.get(i).wcat_id;            // 标签表中的分类ID
		 var wcmbean = WareRPC.getWareCategoryByID(wcat_id);// 根据ID（标签表中的分类ID=分类表ID）取得信息标签分类
		  
		 var wcat_name = wcmbean.wcat_name;	        // 分类名称
		 var wcat_position = wcmbean.wcat_position; //分类层级（分类表ID用$分隔的层级）	
		 if(wcmbean.wcat_level==1)                   // 层级为1 分类名称（wcat_name）为最顶级分类
		 {
			 wcat_position = wcat_name; 
		 }
		//*********
		if(wBeanList.get(i).ware_type == "0"){
			liStr += "<li class='static' onclick='insertWareTag_F(\""+wBeanList.get(i).ware_id+"\",\""+wBeanList.get(i).ware_name+"\",\""+wcat_position+"\")'>"+wBeanList.get(i).ware_name+"("+wBeanList.get(i).ware_id+")</li>";
		}else if(wBeanList.get(i).ware_type == "1"){
			liStr += "<li class='auto' onclick='insertWareTag_F(\""+wBeanList.get(i).ware_id+"\",\""+wBeanList.get(i).ware_name+"\",\""+wcat_position+"\")'>"+wBeanList.get(i).ware_name+"("+wBeanList.get(i).ware_id+")</li>";
		}else if(wBeanList.get(i).ware_type == "2"){
			liStr += "<li class='opt' onclick='insertWareTag_F(\""+wBeanList.get(i).ware_id+"\",\""+wBeanList.get(i).ware_name+"\",\""+wcat_position+"\")'>"+wBeanList.get(i).ware_name+"("+wBeanList.get(i).ware_id+")</li>";
		}else if(wBeanList.get(i).ware_type == "3"){
			liStr += "<li class='rss' onclick='insertWareTag_F(\""+wBeanList.get(i).ware_id+"\",\""+wBeanList.get(i).ware_name+"\",\""+wcat_position+"\")'>"+wBeanList.get(i).ware_name+"("+wBeanList.get(i).ware_id+")</li>";
		}
	}
	*/
	...
	for(var i=0; i<wBeanList.size(); i++){
		wareMap.put(wBeanList.get(i).ware_id,wBeanList.get(i));
		if(wBeanList.get(i).ware_type == "0"){
			liStr += "<li class='static' onclick='insertWareTag(\""+wBeanList.get(i).ware_id+"\",\""+wBeanList.get(i).ware_name+"\")'>"+wBeanList.get(i).ware_name+"("+wBeanList.get(i).ware_id+")</li>";
		}else if(wBeanList.get(i).ware_type == "1"){
			liStr += "<li class='auto' onclick='insertWareTag(\""+wBeanList.get(i).ware_id+"\",\""+wBeanList.get(i).ware_name+"\")'>"+wBeanList.get(i).ware_name+"("+wBeanList.get(i).ware_id+")</li>";
		}else if(wBeanList.get(i).ware_type == "2"){
			liStr += "<li class='opt' onclick='insertWareTag(\""+wBeanList.get(i).ware_id+"\",\""+wBeanList.get(i).ware_name+"\")'>"+wBeanList.get(i).ware_name+"("+wBeanList.get(i).ware_id+")</li>";
		}else if(wBeanList.get(i).ware_type == "3"){
			liStr += "<li class='rss' onclick='insertWareTag(\""+wBeanList.get(i).ware_id+"\",\""+wBeanList.get(i).ware_name+"\")'>"+wBeanList.get(i).ware_name+"("+wBeanList.get(i).ware_id+")</li>";
		}
	}
...
function changeFormModel(val)
{
    ...
    //自定义表单
	//alert(val);
    if(val.indexOf("custom_")>-1)
	{
	   setArrayInFormModelList(info_item);
       var modelCustomCur ;
        for(var i=0;i<modelCustomList.size();i++){
			var modelCustom = modelCustomList.get(i);
			var model_id = modelCustom.model_id;
			if("custom_"+model_id == val){//不是系统默认
					modelCustomCur = modelCustom;
					break;
			}
		}
		if(modelCustomCur.app_id=='zwgk'){//内容管理用
			setArrayInFormModelList(gk_info_item);
		}

		//取出自定义表字段
		var mapForm = new Map();
		mapForm.put("model_id",modelCustomCur.model_id)
		var fieldsCustom =jsonrpc.FormRPC.getFormFieldsListByModelIdN(mapForm);
		fieldsCustom = List.toJSList(fieldsCustom);//把list转成JS的List对象	
		var fieldsCustomStr = new Array();
//var cms_video_item = [["$!{InfoData.video_path}","视频路径"],["$!{InfoData.info_content}","内容"]];
		for(var i=0;i<fieldsCustom.size();i++){
		   fieldsCustomStr[i] = new Array();
		   var field = fieldsCustom.get(i);
           fieldsCustomStr[i][0] = "$!{InfoData."+field.field_enname.toLocaleLowerCase()+"}";
		   fieldsCustomStr[i][1] = field.field_cnname;
		}
		setArrayInFormModelList(fieldsCustomStr);
