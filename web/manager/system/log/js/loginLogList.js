...
var SettingLogsBean = new Bean("com.cicro.wcm.bean.logs.SettingLogsBean",true);
var LogManager = new Bean("com.cicro.wcm.services.Log.LogManager",true);
...
function initTable(){
	...
	colsList.add(setTitleClos("audit_des","操作描述","150","","",""));
...
function showList(){
	...
	beanList = LogSettingRPC.searchLoginLogs(m);//	
...
function showTurnPage(){
	tp.total = LogSettingRPC.searchLoginLogsCnt(m);
	tp.show("turn","");	
...
function roleSearchHandl(obj)
{
	...
	if(start_day.trim() != "" && end_day.trim() != "")
	{
		...
		{
			top.msgWargin("结束时间不能早于开始时间，请重新选择结束时间");
			return;
		}
	}
...
function deleteLogSetting()
{
	...
	{
		top.msgAlert("审计日志"+WCMLang.Delete_success);
		reloadLogList();
	}else
	{
		top.msgWargin("审计日志"+WCMLang.Delete_fail);
	}
}
