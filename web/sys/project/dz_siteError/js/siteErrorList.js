...
var SiteErrorBean = new Bean("com.cicro.project.dz_siteError.SiteErrorBean", true);
...
function openUpdateSiteErrorPage(id) {
    ...
    top.addTab(true,"/manager/project/dz_siteError/viewSiteError.jsp?id=" + c_id +"&topnum="+top.curTabIndex,"站点纠错信息");
}
...
function deleteSiteError() {
    ...
    if (SiteErrorRPC.deleteSiteError(m)) {
        top.msgAlert("站点纠错信息" + WCMLang.Delete_success);
        reloadSiteErrorList();
    }
    else {
        top.msgWargin("站点纠错信息" + WCMLang.Delete_fail);
    }
}
...
function updateSiteErrorData() {
    ...
    */
    ...
    if (bool) {
        top.msgAlert("站点纠错信息保存成功");
        top.getCurrentFrameObj(topnum).reloadSiteErrorList();
        top.CloseModalWindow();
    } else {
        top.msgWargin("站点纠错信息保存失败，请重新操作");
    }
}

function addInfo() {
    top.addTab(true,"/manager/project/dz_siteError/viewSiteError.jsp?topnum="+top.curTabIndex,"站点纠错信息");
}
...
function savePublishFlag(publish_flag) {
    ...
    if (SiteErrorRPC.publishSiteError(m)) {
        top.msgAlert("站点纠错信息状态设置成功");
        reloadSiteErrorList();
    } else {
        top.msgWargin("站点纠错信息状态设置失败，请重新操作");
    }
}
