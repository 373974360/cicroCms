...
var SzbBean = new Bean("com.cicro.project.dz_szb.SzbBean", true);
...
function showList() {
    ...
    table.getCol("preview").each(function (i) {
        if (i > 0) {
            $(this).html('<a target="_blank" href="/manager/project/dz_szb/preview.jsp?id=' + beanList.get(i - 1).id + '" >预览</a>');
        }
    });
...
function openUpdateSzbPage(id) {
    ...
    top.addTab(true,"/manager/project/dz_szb/viewSzb.jsp?id=" + c_id +"&topnum="+top.curTabIndex,"数字报信息");
}
...
function addInfo() {
    top.addTab(true,"/manager/project/dz_szb/viewSzb.jsp?topnum="+top.curTabIndex,"数字报信息");
}
