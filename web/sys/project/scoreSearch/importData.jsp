...
<head>
    ...
    <SCRIPT LANGUAGE="JavaScript">
        ...
        $(document).ready(function () {
        	...
        	var examList = jsonrpc.ExamRPC.getAllExamList();
        	examList = List.toJSList(examList);//把list转成JS的List对象
        	if(examList != null && examList.size() > 0){
        		for(var i = 0 ;i < examList.size(); i++){
        			var examBean = examList.get(i);
        			$("#examId").append("<option value='" + examBean.id + "'>" + examBean.name + "</option>");
        		}
        	}
        ...
        function fileSave(path)
        {
        	if($("#examId").val() == ""){
                ...
                if(jsonrpc.ScoreRPC.importScoreData(path,$("#examId").val()))
                {
                    top.msgAlert("数据导入成功");
                }
                else
...
<form id="form1" name="form1" action="#" method="post">
    <div class="people_table_content">
        <table id="death_table" class="table_form" border="0" cellpadding="0" cellspacing="0">
            ...
            <tr>
                <th>考试分类：</th>
                <td ><select class="input_select width90" name="examId" id="examId">
                    </select>
                </td>
            </tr>
            <tr>
