...
function showList(){			
	...
	table.getCol("domain_col").each(function(i){	
		if(i>0)
		{			
			$(this).html('<a href="javascript:top.addTab(true,\'/sys/control/site/domainList.jsp?site_id='+beanList.get(i-1).site_id+'\',\'域名管理\');">域名管理</a>');			
		}
	});
...
{  
	...
	if(create_flag == "true")
	{
		top.addTab(true,'/sys/control/site/site_add.jsp?parentID='+site_id+"&tab_index="+top.curTabIndex,'维护站点');
	}else
	{
...
{  
	...
	top.addTab(true,'/sys/control/site/site_add.jsp?site_id='+selectIDS+'&parentID='+site_id+"&tab_index="+top.curTabIndex,'维护站点');
}

function openViewPage(s_id)
{  
	top.addTab(true,'/sys/control/site/site_view.jsp?site_id='+s_id+"&tab_index="+top.curTabIndex,'查看站点');
}
