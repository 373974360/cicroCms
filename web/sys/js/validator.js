...
Validator = function(){
	...
	this.checkStrLength=function (value,name,maxnum,input_name){	
		if (value != "" && value != null) {				
			...
			{
				this.check_result = false;
				this.addError(input_name, "【" + name + "】项输入的内容字数不能大于"+maxnum+"字符,"+parseInt(maxnum/3)+"个汉字");
				return false;
			}
			else
				return true;
	...
	this.checkDomain=function (value, name,booleanFlag,input_name){
		...
		//var rWebsite = /^[0-9a-z]+\.[0-9a-z\-\_]+\.[0-9a-z\-\_]+(\.[0-9a-z]+)?$/;
		var rWebsite = /^[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+\.?$/;
		var bVal = rWebsite.test(value);
		if (bVal) {
			return true;
		}
	...
	//判断是否由字母、数字、下划线组成，并且仅能字母开头，不以下划线结尾组成
	this.checkLetterL = function(value,name,booleanFlag,input_name){
		if(booleanFlag && (value == null || (value = $.trim(value)).length == 0)) 
			return true;
	    var Val = null2String(value);
		if (Val != "") {
			var rIP = /^([a-zA-Z]|[a-zA-Z][a-zA-Z_0-9]*[a-zA-Z0-9])$/;
			var bVal = rIP.test(Val);
			if (bVal) {
				return true;
			}
			else {
				this.check_result = false;
			    this.addError(input_name, "【" + name + "】格式不正确！请按提示要求填写！");
				return false;
			}
		}else{
			return true;
