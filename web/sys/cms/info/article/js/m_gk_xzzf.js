var GKFXzzfBean = new Bean("com.cicro.wcm.bean.cms.info.GKFXzzfBean",true);
...
//	}
	var bean = BeanUtil.getCopy(GKFXzzfBean);	
	$("#info_article_table").autoBind(bean);
	if(!standard_checkInputInfo("info_article_table"))
	{
		return;
	}
	bean.gk_xzxw = getV(gk_xzxw);
	bean.gk_xzcl = getV(gk_xzcl);
	bean.gk_flyj = getV(gk_flyj);
	...
	publicSaveInfoEvent(bean,"gkfxzzf","insert");
}
//修改
...
//	}
	...
	publicSaveInfoEvent(bean,"gkfxzzf","update");
}