    ...
    <div class="con">
            ...
            <div class="con_left2">
                ...
                <div class="right">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <span class="span1"><a href="#">上一版</a></span>
                        </c:otherwise>
                    </c:choose>
                    <c:choose>
        ...
        <div class="con_right left">
            ...
            <div class="text_con">
                ...
                <div>
                    <span class="text_con1"><c:if test="${article.source!=''}" ><span>来源：&lt;&lt;${article.source}&gt;&gt;</span></c:if> <span>(<i
                            class="margin_right">${article.released_dtime}</i>)</span></span>
                    ${article.info_content}
                </div>
                <p>
                    <c:choose>
                        <c:when test="${j>0}">
                            <a class="span3" href="./article.jsp?id=${bean.id}&i=${show}&j=${j-1}&a=${bean.json[show].areas[j-1].id}">上一篇</a>
                        </c:when>
                        <c:otherwise>
                            <span class="span3">上一篇</span>
                        </c:otherwise>
                    </c:choose>

                    <c:choose>
                        <c:when test="${f:length(bean.json[show].areas)>j+1}">
                            <a class="span4" href="./article.jsp?id=${bean.id}&i=${show}&j=${j+1}&a=${bean.json[show].areas[j+1].id}">下一篇</a>
                        </c:when>
                        <c:otherwise>
                            <span class="span4">下一篇</span>
                        </c:otherwise>
                    </c:choose>
